...
Fourteen tools over the persistent semantic index: `fuse_index` (build or refresh the index), `fuse_map` (workspace map of symbols, routes, and counts), `fuse_localize` (rank candidate files for a task, with the graded signal-sufficiency contract), `fuse_resolve` (resolve wiring: service to implementation, request to handler, route to action, config to options), `fuse_context` (emit scoped, reduced source with provenance for selected seeds), `fuse_review` (diff-first change impact and packed context), `fuse_find` (exact symbol, path, or text lookup), `fuse_neighbors` (the graph neighborhood of a file, the callers and implementers of a symbol, or the central files of an area), `fuse_signatures` (batch exact signatures for named symbols in one call, from the symbol store), `fuse_impact` (blast radius for a symbol: callers, implementers, and referencing types from the persisted graph; abstains on the tier-1 signature-change break set), `fuse_check` (speculatively typecheck a proposed single-file edit against the build-captured compilation, with R6 repair packets on API-shape diagnostics; oracle-grade, abstains otherwise), `fuse_refactor` (compiler-executed solution-wide rename staged as a diff; abstains on a partial load), `fuse_changeset` (the speculative staging area: create/stage/diagnose/select/promote/discard a changeset, writing to the tree only on an explicit promote), and `fuse_reduce` (compact a known set of files or raw content). Read tools build the index on first use. MCP resources cover the map, localize, context, and review workflows.
