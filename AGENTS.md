...
Token reduction is now a rendering and transport feature rather than the product claim: `fuse review` delivers a PR's context in a median 874 tokens. Warm latency, a full token-reduction and fidelity re-measurement on the V3 surface, peer-scoper comparison, and full task resolution (patch plus test oracle) are not re-run on V3 here; treat any such claim as illustrative until recorded.
