...
- Semantic resolution (Suite A, `semantics.json`): the extracted graph matches the hand-built edge ground truth 100 percent (14 of 14 edges, recall and precision 1.0) on the OrderingApp wiring fixture, which covers DI registration and constructor injection, MediatR request-to-handler, ASP.NET route-to-action, options binding and consumption, and (R5) hosted-service workers, MediatR pipeline behaviors, and EF Core DbContext-to-entity and entity-to-configuration wiring. Open and closed generic DI is resolved through the existing typeof-pair registration path. This is the deterministic moat; R5 widens it with edge kinds a tree-sitter or lexical index cannot follow.
