...
- Open-ended localization (Suite C, `localize.json`, 53 PRs, title only, `--restore`, dense on by default): overall changed-file recall about 15 percent at 8.1 percent precision, median 1,033 returned tokens; by bucket identifier-rich 21 percent, natural-language domain 17 percent. Contract metrics: low-signal detection F1 1.0, false-rejection on answerable queries 0.0 percent, precision when confident 5.6 percent over the 9 confident tasks (from the current `localize.json`; the earlier 9.3 was the superseded a1 run). The dense channel (default on, offline, fetched once and cached) lifts recall over the lexical fallback (13.3 to 14.9 percent overall, `localize.a1-lexical.json` vs `localize.json`) and removes false rejections; with no model present the path is byte-identical to the lexical fallback. The weakest mode, bounded by a mostly-syntax corpus (localize main-checkout modes partial 2, syntax 2). The retrieval levers (subword indexing, stemming, a comment field, dependency-centrality and git co-change priors, opt-in graph expansion) operate over the language-agnostic tables.
- Tier-1 localize re-run (`localize.tier1.json`, v4 N4 Phase 4 gate): with tier-1 build capture enabled, open-ended localize recall is 15.0 percent versus the baseline 14.9 percent (`localize.json`), within CI, with the index-mode distribution unchanged (partial 2, syntax 2). The honest read: oracle-grade build capture produces a richer, build-exact graph on the buildable repos but does not move open-ended localize recall on this corpus, so it is positioned as a verification asset (the substrate for `fuse_check`, `fuse_impact`, covering-test selection), not an open-ended-recall lever. The retrieval bets V1/V2 are therefore not warranted by this evidence.
- Ranking regression (`ranking.json`, 53 PRs, changed-file truth, corpus main checkouts partial 2 / syntax 2): the ranking gate. It scores MRR, recall@k, and nDCG@k in three configurations. Recorded 2026-07-03: lexical channel MRR 0.187, recall@10 12.6 percent, nDCG@10 0.117; shipping default (dense plus priors) MRR 0.197, recall@10 15.0 percent, nDCG@10 0.139. The FTS5 field-weight inversion is fixed (name and signature and symbols now outrank path, guarded by this suite). A6 re-adjudication: the git co-change prior is slightly MRR-negative here (default MRR 0.197 versus default-without-co-change 0.208) and recall@10 flat, weakly confirming finding 9's direction; the delta is within CI on a mostly-syntax corpus, so the default is held on pending N4's richer-tier re-run rather than flipped on noise. The suite is the required gate on any change to field weights, tokenization, query expansion, or priors.
