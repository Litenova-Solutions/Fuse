...
- **The eight-tool loop surface (U1, in progress).** The MCP tool surface is being reshaped from fifteen flat tools into a loop-shaped surface of eight tools, each one mental act, plus `fuse_reduce` as the one out-of-loop utility. `fuse_workspace` (status, index, map, doctor, and the D2 `apply` write path) folds `fuse_index` and `fuse_map`; `fuse_find` becomes a typed union (`kind=symbol|path|text|all` plus `service|request|route|config|signatures|neighbors|task`) folding `fuse_localize`, `fuse_resolve`, `fuse_neighbors`, and `fuse_signatures`; `fuse_changeset` dissolves into `fuse_check` (diagnose an edit by passing content), `fuse_refactor` (compiler-executed edits), and `fuse_workspace action=apply` (write). Behavior changes: every folded name (`fuse_index`, `fuse_map`, `fuse_localize`, `fuse_resolve`, `fuse_neighbors`, `fuse_signatures`, `fuse_changeset`) stays registered as a deprecation shim naming its replacement, so a client that cached the old surface across an upgrade gets an actionable message rather than an opaque Unknown tool error (reconnect the MCP server to refresh the tool list); the server's one explicit tree-write path is now `fuse_workspace action=apply` (Decision D2), a dry run unless `write=true` and refusing any path that escapes the workspace root. No host RPC protocol change (these are MCP tools, not the host contract). The `ServerInstructions` now teach the loop.

### Added

- **`fuse up --apply`: the install-free environment remedy, applied (C1).** `fuse up` gains an `--apply` flag that applies the NU1507 remedy (Central Package Management with multiple sources but no source mapping) and re-attempts the load: it writes the overlay NuGet config to a temp file, runs `dotnet restore --configfile <overlay>` (never editing the repository, per the C1 hard rule), then re-runs the load diagnosis and prints the new per-project tiers. It is install-free (the overlay only supplies a source mapping; it downloads nothing a normal restore would not) and bounded to a single remediation round (no unbounded retry). The consent-gated install remedies (SDK via `NETSDK1045`, workload via `MSB4018`) are never run without `--allow-install`; without it they are reported, not applied. New engine `EnvironmentRemediationApplier` (in `Fuse.Semantics`) with the integration test the item names: a synthetic CPM project with two sources and no mapping reproduces NU1507 on a bare restore, and the overlay restore removes it. Behavior change (CLI): `fuse up` gains `--apply` and `--allow-install`; the default remains report-only and repository-safe. This is the first C1 apply step; the auto-apply across the full remediation set and the corpus-scale `up-report.json` Gate remain (the Gate needs a corpus of repositories that actually fail, since the current pinned corpus builds clean).

- **Agent panel working-tree diff and handoff preview (G3b, completes G3).** The Agent Sessions panel gains a workspace-level **Working tree (vs HEAD)** node (a sibling of the session rows, since the working tree is one tree, not per session): it lists the files the uncommitted edits changed with their added/removed line counts (click to open) and a paste-ready handoff preview. A git-free "files touched" summary was also added under each session, computed from the diagnostics the panel already has. Behavior change (host RPC contract): a new read-only `fuse/session-diff` method returns the `git diff HEAD` file set (via the existing change source) plus the handoff packet (reusing `BuildHandoffAsync` against HEAD); `FuseHostService.ProtocolVersion` bumps 5 to 6 and the extension's `protocol.ts` `PROTOCOL_VERSION` mirrors it in the same change, with the DTO shapes (`SessionDiffDto`, `SessionDiffFileDto`) pinned by the .NET and TypeScript contract suites in lockstep; reload the VS Code window after upgrading. The method is best-effort (a missing git or HEAD returns an unavailable result, so the panel shows a note rather than erroring). This completes the G3 Ships list (the diff and handoff-preview views the diagnostics-only panel deferred under the G3 Fallback).

- **VS Code agent observability panel (G3).** The extension gains an **Agent Sessions** panel: the read-only human window onto what an agent is doing. It lists the sessions the host knows for the workspace and, on expanding one, shows the compiler diagnostics that session's edits introduced or resolved since its baseline (live, from the resident workspace) and its graded claim ledger (the U2 evidence trail). It is strictly read-only - no promote or discard actions (those need the F1 transaction boundary). Two new read-only host RPC methods back it, `fuse/sessions` (list) and `fuse/session-view` (per-session diagnostics + rendered claims), plus a store `ListSessionsAsync` enumerator over the existing `check_sessions` and `claim_ledger` tables (an additive read, no schema change). Behavior change (host RPC contract): `FuseHostService.ProtocolVersion` bumps 4 to 5 and the extension's `protocol.ts` `PROTOCOL_VERSION` mirrors it in the same change, with the DTO shapes pinned by the .NET and TypeScript contract suites in lockstep; reload the VS Code window after upgrading so the extension re-handshakes with the new host. The panel's data mapping is covered by a headless fixture-driven test. The git-dependent staged-diff and handoff-preview views are a named follow-up (G3b), deferred under the item's Fallback because spawning git inside the host process is environmentally fragile.

- **Playbook prompts, session resources, and CLI parity (U3).** The MCP server now exposes five playbook prompts - selectable, anchored plans that teach the verified-edit loop without a separate skill document: `fix-build-error` (anchor: diagnostic id), `implement-feature` (issue, optional git base), `review-pr` (merge base), `rename-symbol` (symbol FQN), and `add-endpoint` (route prefix). Each expands into an ordered walk of the eight-tool loop. Three new addressable resources join the U2 session ledger: `fuse://status/{path}` (workspace status), `fuse://diff/{path}/{session}` (the diagnostics a session's edits introduced or resolved since its baseline - the read-only, idempotent form of `fuse_check --delta`, which never establishes a baseline), and `fuse://diagnostics/{path}/{session}` (a session's whole-state diagnostics: the live resident set, or the recorded baseline). CLI parity closes the gap between the agent verbs and the command line: a new `fuse impact <symbol>` command (blast radius, plus the F3 package-upgrade mode via `--package`/`--from-version`/`--to-version`) and `--handoff`/`--check-session` on `fuse review` (the paste-ready PR packet with the red-gate); `fuse check --delta` and `fuse test` were already CLI-first. New engine pieces `FusePrompts` (registered via `.WithPrompts<>`), the three resource reads on `FuseResources`, and `ImpactCommand`; `FuseTools.BuildHandoffAsync` is now internal so the review command reuses the exact handoff builder. New docs: the playbook prompts and session resources on the [resources reference](https://fuse.codes/docs/reference/mcp-resources), and a [CI recipe page](https://fuse.codes/docs/scenarios/ci-recipe) running review plus test on a pull request. No host RPC protocol change (MCP prompts/resources and CLI commands, not the host contract).

- **`fuse_find kind=signatures` resolves referenced-assembly metadata when resident (U1b, completes U1).** When a live resident workspace serves the root, a signature query for a qualified name (a namespace-qualified type, or `Type.Member`) is answered from the held compilation's real metadata rather than the store, so a referenced package's API returns the compiler's own view of the shipped surface instead of a store row that never indexed the package (or a guess) - the hallucinated-package-API check. A qualified type lists the type and its public and protected members; a `Type.Member` query lists the matching overloads; a simple name resolves the project's own source declarations only (a metadata assembly cannot be searched by simple name without a full walk, so metadata resolution needs a qualified name). Without a resident workspace, or for a name the resident path does not resolve, the store-backed signature index answers exactly as before. New additive seam `IResidentWorkspaceProvider.TryGetSignature` (default null) and `ResidentWorkspace.GetSignatures` (querying `GetTypeByMetadataName` and `GetSymbolsWithName` over the held compilations), returning a new `ResidentSignature` record. No behavior change without a resident workspace (the default null provider returns null); no host RPC protocol change.

- **Graded claims, the session evidence ledger, and the PR handoff packet (U2).** The statements Fuse emits inside an answer are now graded by the evidence behind them, so an agent and its human know which are compiler-grade truth and which are graph-grade inferences. Four tools carry a claims block, appended as a scannable text section (the read tools return rendered text, so a claim is a section like the availability header, not a new envelope): `fuse_impact` (caller/implementer and covering-test counts, graph-grade), `fuse_find` wiring kinds (the resolved edge, graph-grade), `fuse_test` (covering tests run and their verdicts, compiler/test-grade `verified`), and `fuse_review` (the changed-file set, git-truth `verified`, plus a `partially verified` claim when the public API surface moved). Grades are computed, never asserted: `verified` requires compiler- or test-grade evidence; a graph-only answer caps at `partially verified` (the grade-inflation guard); `stale` fires when a claim's evidence file changed since it was computed; `contradicted` fires when a session claim conflicts with the current truth, both sides cited. Across a session the claims accumulate into a ledger addressable as a new MCP resource (`fuse://ledger/{path}/{session}`). `fuse_review --handoff` turns the accumulated evidence into a paste-ready PR body (changed files, API delta, compiler-gate status, named residual risk); it is gated, not a controller - while the `checkSession` has unresolved introduced errors it refuses with the red summary rather than emitting a packet. Behavior changes (tool surface): `fuse_impact` and the wiring `fuse_find` kinds, `fuse_test`, and `fuse_review` gain the claims section; `fuse_review` gains `handoff` and `checkSession` parameters; the JSON context format gains a nullable `claims` field so the payload stays valid JSON. New engine pieces `Claim`, `ClaimGrade`, `ClaimReviewer`, `ClaimLedger`, and `SessionClaimLedger` (in `Fuse.Retrieval`), persisted through a new additive `claim_ledger` table (idempotent schema DDL, no version bump, no rebuild). No host RPC protocol change (these are MCP tools). New concepts page [claim grades](https://fuse.codes/docs/concepts/claim-grades); golden outputs pin the claims-block shape.

- **NuGet upgrade oracle: package-bump break prediction (F3).** `fuse_impact` gains a package-upgrade mode: pass `package`, `fromVersion`, and `toVersion` and it diffs the public API of the two package versions (resolved from the local NuGet cache) and lists the breaking changes the bump would introduce (removed members, signature changes, accessibility reductions), so a package upgrade's risk is knowable before the lockfile changes. It reuses the T2 public-API delta over a new metadata surface extractor (loading the assembly as a `MetadataReference` and walking its public symbols), abstains with a reason when a version is not cached (the offline case), and names its blind spots on every report (reflection/dynamic usage, and that repo call-site intersection is bounded by the reference graph, which does not track external-package call sites). New engine pieces `MetadataSurfaceExtractor` (in `Fuse.Semantics`) and `PackageUpgradeOracle` (in `Fuse.Retrieval`). The zero-false-safe contract is validated on real cached version pairs: `System.Text.Json` 4.7.2 to 8.0.0 is flagged breaking (the public `JsonClassInfo` was removed) while additive-only major bumps of `System.Collections.Immutable` and `Microsoft.Extensions.DependencyInjection.Abstractions` report no break.

- **CI parity rehearsal: `fuse verify --ci-parity` (G8).** A new command reads the repository's GitHub Actions workflows, extracts the dotnet command sequence they run (single-line `run:` and `run: |` blocks, best-effort and dependency-free), prints it in CI order, and classifies the steps that cannot be rehearsed locally as named classes (a step needing a secret, a `dotnet nuget push`), never silently skipping any. With `--run` it executes the rehearsable steps locally through the same executor build-grade verification uses (only a clean leading-`dotnet` command is run; a wrapped invocation is reported, not auto-run). The contract is classification, not emulation: it names what CI will run and what this machine cannot reproduce, so a "local green, CI red" surprise has a named reason first. New engine pieces `CiWorkflowParser` and `CiParityRehearser` (in `Fuse.Workspace`). Verified on the corpus: eShopOnWeb (3 rehearsable dotnet steps, 0 non-rehearsable) and Scrutor (3 rehearsable, 2 secret-bearing pushes named non-rehearsable).

- **Keyed dependency-injection coverage in wiring resolution (G2).** The DI registration analyzer now recognizes the keyed registrations (`AddKeyedScoped`, `AddKeyedSingleton`, `AddKeyedTransient`, and their `TryAddKeyed*` variants), so a service registered with a key resolves to its implementation in the semantic graph exactly as a non-keyed registration does (the service key is a value argument that does not change the service-to-implementation edge). The Suite A wiring ground truth grew from 22 to 23 edges with the keyed case added and stays exact (recall and precision 1.0, zero false positives). This is the first iteration of the repeatable analyzer-pack coverage growth, whose contract is that an analyzer that cannot reach exactness on its fixture does not merge.

- **Machine-applicable repair on check packets, and DiagBench (H2).** A `fuse_check` repair packet for an unambiguous token-level API-shape error (a misspelled member `CS1061`/`CS0117` or type name `CS0246`) now carries a structured, machine-applicable `TopRepair` - the offending token and the nearest recorded name to substitute - not only the prose suggestion, and `fuse_check` renders it as an `apply: replace 'X' with 'Y'` line so an agent (or a hook) can apply the fix without re-deriving the edit from the explanation. A diagnostic with no single-token fix (`CS7036` missing argument, `CS0029` type mismatch) carries no `TopRepair`. This is additive to the `RepairPacket` record (a nullable field) with no schema or protocol change. The new `fuse eval diagbench` suite grades the packet protocol itself: over an API-shape mutant set it auto-applies each packet's `TopRepair`, recompiles, and records the auto-fix rate per diagnostic id to `tests/benchmarks/results/diagbench.json` - a deterministic feedback number for the repair-packet work (the product never auto-applies; this is a benchmark-harness path only).

- **Type-level and code-fix refactors, verify-gated (T4, T4b).** `fuse_refactor` gains three more operations. `extract-interface` generates an interface from a class's public instance methods and properties (named with `newName`, else `I<Class>`) and makes the class implement it; `move-type` moves a top-level type to its own new file named after it, removing it from its current file; `apply-codefix` applies the repository's own analyzer code fix for a `diagnosticId` in a `file`, driving that diagnostic to zero (it discovers the analyzers and their `[ExportCodeFixProvider]` fixes from the project's analyzer references by reflection, runs the analyzer, and applies the fix to each occurrence). All three recompile the solution and return the staged change only when no new diagnostic is introduced, abstaining otherwise. New engines `TypeRefactorer` and `CodeFixApplier` (in `Fuse.Semantics`). The full refactor family is now rename plus add/remove/reorder-parameter, add-cancellation-token, extract-interface, move-type, and apply-codefix.

- **Constrained change-signature, verify-gated (T3, T3b).** `fuse_refactor` grows from rename-only into a verify-gated change-signature family via an `operation` switch: `add-parameter` (add a trailing parameter to a method and its whole override/interface family, threading an explicit argument into every call site), `add-cancellation-token` (add a `CancellationToken` and thread an in-scope token into every caller that has one, listing token-less sites as manual follow-ups), `remove-parameter` (drop a parameter and its call-site arguments, abstaining when the parameter is used in a body or a call site passes a possibly side-effecting argument), and `reorder-parameters` (reorder into a given order, abstaining on any positional call site). Every signature operation is correctness-by-verification: after a best-effort Roslyn rewrite it recompiles the solution and returns the staged diff only when no new diagnostic is introduced, otherwise abstaining with the offending sites named - never a mostly-right diff. New engine `ChangeSignatureRefactorer` (in `Fuse.Semantics`). Behavior change (tool surface): `fuse_refactor` gains `operation`, `containingType`, `parameterType`, `parameterName`, `argument`, and `newOrder` parameters; the default `rename` behavior is unchanged. The 25-case matrix (`tests/benchmarks/results/changesig.json`) records 0 diffs that fail compilation and a 32 percent abstention rate.

- **Analyzer and nullable parity in check (S4).** When a resident workspace serves the root, `fuse_check` now also runs the repository's configured analyzers (StyleCop, the .NET code-analysis rules, package analyzers) and nullable warnings against the overlay compilation at the repository's editorconfig severities, and merges them with the compiler diagnostics, so a green check matches what CI's build step enforces rather than only the raw compiler errors. An editorconfig-elevated rule surfaces; a silenced rule stays silent. The analyzers and their editorconfig-mapped options are captured from the tier-1 build log onto each resident project; the check forks the held compilation for the proposed edit and preserves the edited document's editorconfig severities across the fork (a `SyntaxTreeOptionsProvider` redirect, without which a replaced tree loses its per-tree severity mapping). New `analyzers` parameter on `fuse_check` (default on for the single-file verify, since an explicit check tolerates the analyzer latency for CI parity; the hot per-edit delta path stays analyzers-off, since the measured analyzer cost on a large set is hundreds of milliseconds). The build-grade fallback already had this parity for free, because it runs the real `dotnet build`. New engine pieces: `ResidentAnalyzerRunner` and the analyzer-aware `ResidentWorkspace.CheckOverlayAsync`, threaded through a new async `IResidentWorkspaceProvider.TryCheckOverlayAsync`. No effect without a resident workspace. Analyzer cost recorded in `tests/benchmarks/results/resident-latency.json` (P95 886.9 ms for 284 analyzers on NodaTime).

- **Ambient verification: harness hooks that push the diagnostics delta and gate a red turn (S3, in progress).** Two new CLI commands turn verification from pull-only into ambient: `fuse check --delta [--session id] [--fast]` prints the compiler diagnostics the session's on-disk edits introduced or resolved since its baseline (silent on an empty delta, so no transcript spam), and `fuse gate` exits nonzero with a red summary while the session has introduced errors it has not resolved (baseline discipline: only errors the session itself introduced block, never pre-existing repo errors). Both connect to an already-running `fuse host` / `fuse mcp serve` over the existing UI transport via a new `fuse/check` RPC and never run a build; with no host serving the root they exit 0 silently, so a hook never blocks editing. `fuse mcp install --with-hooks` writes the matching Claude Code hooks into project `.claude/settings.json` (PostToolUse on Edit/Write runs `fuse check --delta --fast`; Stop runs `fuse gate`), preserving other settings and idempotent on re-run. Behavior changes: the host RPC protocol bumps to 4 (the `fuse/check` method and `CheckDeltaDto`/`CheckDiagnosticDto` shapes; the extension `protocol.ts` mirror and both contract suites move in lockstep); `fuse` gains the `check` and `gate` commands and `fuse mcp install` gains `--with-hooks`. A no-host probe adds no measurable connect latency (a `\\.\pipe\` existence pre-check short-circuits it), so the command cost is dotnet start-up alone. Remaining S3: the dual-shell scripted end-to-end and the ambient-verification docs page.

- **`fuse_check` delta mode, persisted sessions, and repair packets v2 (S2, in progress).** `fuse_check` gains a delta mode: pass a `session` id with no `content` and it returns the diagnostics your on-disk edits introduced or resolved since the session baseline (the inverse of the speculative single-file check), each introduced error carrying a repair packet where one applies. The baseline is set on the first call and persists to the store (a new additive `check_sessions` table via the idempotent schema DDL, so no version bump and no rebuild), so a restarted process resumes the session with its baseline intact; `markGreen: true` resets the baseline to the current state and `full: true` returns the whole current diagnostic set. Delta mode never runs a build (a `DiagnosticDelta` over two diagnostic sets, matching by id and message with the line excluded so an edit that shifts a diagnostic does not read as a phantom change); it reads the current whole-state diagnostics from a live resident workspace via a new `IResidentWorkspaceProvider.TryGetCurrentDiagnostics` seam and abstains, naming the `FUSE_RESIDENT` prerequisite, when none serves the root. Repair packets expand beyond CS1061/CS0246/CS0117 to CS7036 (a missing required argument, surfacing the callee's recorded signature) and CS0029 (a wrong-type assignment, naming the explicit-cast direction and any source-type member that yields the target). Behavior change (tool surface): `fuse_check` gains `session`, `full`, and `markGreen` parameters; the existing single-file content path is unchanged (byte-identical when `content` is supplied). Remaining S2: the delta-p95 latency re-measurement through the wired tool.

- **Public API delta on review and impact (T2).** `fuse_review` now opens its manifest with a public API delta section: the public and protected members added, removed, or changed between the git base ref and the working tree, each flagged breaking (a removal, a signature change, or an accessibility reduction) or additive; a purely internal change shows no section. The surface is compared by extracting the public and protected symbols of each changed C# file on both sides, the base side read from the base ref (a new `git show <ref>:<path>` capability on the change detector, surfaced as `IChangeSource.GetFileContentAtBaseAsync`) and the current side from the working tree, so a contract break is the first thing a reviewer reads rather than something CI discovers. `fuse_impact` gains a public API surface line: whether the target symbol is on the public or protected surface, so an agent knows before editing whether the change is contract-relevant. Flagging is conservative (the T2 kill-risk mitigation): a positive "public" is asserted only from the recorded public-API flag, reliable for types in any index mode and for members at the semantic tier, and a member in syntax mode (where accessibility is unresolved) is reported undetermined rather than guessed. New engine cores: `PublicSurfaceExtractor` (syntax-only public/protected type and member extraction with effective accessibility, in `Fuse.Semantics`), `PublicApiDelta` and `ApiDeltaReport` and `ChangedFileApiDelta` and `ChangedApiSurfaceGatherer` (in `Fuse.Retrieval`). Behavior change (tool output shape): the `fuse_review` manifest and `fuse_impact` response gain the sections above; in the JSON review format the section rides a dedicated `apiDelta` field so the payload stays valid JSON (the `ContextJsonDto` gains a nullable `ApiDelta` field, omitted when absent). Out of scope, named in docs: binary-compatibility subtleties that are not a source-visible surface change (default parameter value changes, `const` inlining). The 10-PR corpus adjudication gate is the remaining validation.

- **Resident workspace foundation (S1, in progress).** A new `Fuse.Workspace` core library introduces `ResidentWorkspace`: it rehydrates a tier-1 build capture (a binary log) once and holds the per-project Roslyn compilations in memory, so a proposed edit can be typechecked against a live compilation via an in-memory overlay (`CheckOverlay`) with no build and no disk write; it also applies edits (`ApplyEdit`) and deletions (`RemoveDocument`) to the retained compilations and exposes the whole-state diagnostics baseline (`GetDiagnostics`) that a delta check diffs against. This is the substrate the incremental watcher, delta check, and session overlays will stand on (replacing the rehydrate-then-discard pattern of the one-shot build-capture worker). The library references the `Basic.CompilerLog` rehydration closure and never `MSBuildWorkspace`, so a process hosting a resident workspace loads only the rehydration closure (Decisions D7/D8). Behavior change (tool output shape): the availability header store-backed read tools prepend now names which truth answered, `workspace store-backed` by default or `workspace resident (N project(s), current as of ...)` once a resident workspace is wired for the root, via a new `IResidentWorkspaceProvider` seam (default reports store-backed, so behavior is unchanged until a host wires a resident engine). `fuse_check` also routes resident-first through the same seam: when a live resident workspace serves the root it answers the oracle-grade check from the held compilation with no build; with the default provider this is a no-op and the build-capture-worker and build-grade paths are unchanged. The remaining S1 sub-steps (the file watcher driving the incremental updates, the serve/host wiring that constructs the resident workspace and populates the provider, and the latency gate) are still to come.

- **Verification-grade ladder: verify never shrugs (T0, Decision D11).** `fuse_check` no longer abstains when tier-1 build capture is unavailable; it falls back to a **build-grade** answer. Every `fuse_check` response is now stamped with a `verification grade`: `oracle` (a speculative in-memory typecheck against the tier-1 build-captured compilation, sub-second class, unchanged), `build` (Fuse runs the real `dotnet build` toolchain scoped to the owning project and parses its diagnostics into the same shape, ground truth at build latency, with the elapsed time reported), or `abstain` (returned only when even the toolchain cannot run - no buildable project, or a build that exceeds the 240 s timeout - always naming the missing prerequisite). The build-grade path never writes the working tree: the owning project is mirrored to a temporary directory, the proposed content replaces the one file in the copy, its `<ProjectReference>` includes are rewritten to absolute paths pointing at the untouched originals, and the build runs there (Decision D2). The one-line availability header store-backed tools prepend now names the grade the workspace can currently serve ("verify serves oracle-grade" or "verify serves build-grade"). New engine type `BuildGradeChecker` (in `Fuse.Semantics`); `CheckResult` gains a `Grade` field (additive, defaults to `oracle` so existing output and worker JSON deserialize unchanged). The oracle-versus-build honesty is measured directly: `fuse eval checkgate --verify-agreement N` (with a build-capture worker configured) runs a sample of mutants through both paths and records their diagnostic-identity agreement to `results/checkgate.json`. Behavior change: on a repo without tier-1, `fuse_check` returns a real red/green verdict at build latency instead of "cannot verify". Scoped to `fuse_check`; carrying the grade into `fuse_review` and `fuse_changeset diagnose` is a named follow-up.

- **Mutation-derived check-honesty calibration at scale (H1).** The `checkgate` suite (Suite F) grows from eight hand-built cases to a mechanically generated battery: `fuse eval checkgate --mutations N` runs N compiler-verified breaking and N neutral single-file mutants per class per fixture through the shipped `CheckResult.IsClean` classification, so "check never lies" is a measured rate rather than an asserted one. A new `MutationGenerator` (Roslyn syntax rewriters) produces the edits; each is labeled by the compiler, not by hand (a breaking mutant is kept only when the compilation reports an error located in the edited file, a neutral mutant only when the compilation stays clean), and generation is deterministic from a recorded seed. Operators are single-file, matching the shipped `fuse_check` contract. The recorded run (`--mutations 500`, `results/checkgate.json`) is 1,000 verified cases (500 breaking, 500 neutral) over the OrderingApp fixture with zero false green and zero false red; the eight curated cases remain a named subset. SampleShop is recorded skipped (its two-project MVC structure does not bind in the flat in-process compilation), not fabricated. No behavior change to `fuse_check` itself; this is a measurement addition (new `--mutations` eval flag).

### Removed

- **Deleted the in-memory BM25F ranker and the dead classic query scoping path (K2, completes 4.0 N2).** `Bm25RelevanceIndex` (a four-field in-memory BM25F ranker, the only `IRelevanceIndex` implementation) is removed, so the tree carries one ranker: the FTS5 table behind `SemanticRetrievalEngine`. A non-shipping ranker that could disagree with the shipping one is a defect class, now gone. The classic fusion query mode it served was already unreachable (no CLI, MCP, or host surface set `FusionRequest.Query`; the two helpers that could, `FusionRequestComposer.ApplyExclusiveScope` and `FusionScopeDescriptor.ApplyMode`, had zero callers), so the whole dead path is removed with it: `QueryScopingPipeline`, `IRelevanceIndex`, `RelevanceIndexCache`, `PseudoRelevanceExpander`, `RankFusion`, `DistributionalThesaurus`, `HeuristicQueryRewriter`, `RelevanceTokenizer`, `QueryExpansionOptions`, `IndexedDocument`, the Fusion-side `CommentExtractor`, `IRelevancePostingsStore`/`SqliteRelevancePostingsStore`, and `QueryOptions`, plus the `FusionRequest.Query` member and its validation. The VS Code host `fuse/scope` already served from `SemanticRetrievalEngine`, so there is no host RPC or DTO change and no protocol bump (host protocol stays 3; the extension contract suite passes unchanged, 8 of 8). The shipping reduce, context, review, focus, and change paths are untouched. No migration required.

- **Retired the dense embedding retrieval channel and the ONNX plugin (K1).** The dense candidate channel (a per-chunk all-MiniLM-L6-v2 embedding persisted at index time and ranked by cosine at query time) is deleted, not demoted: the whole `Fuse.Plugins.Rerank.Onnx` project is removed, so the tool package carries no model, fetches no model, and has no ONNX Runtime dependency. Its recorded contribution did not pay for a 23 MB model download, an index-time embedding step, and a supply-chain surface (the localize A/B measured dense at 14.9 percent recall versus the lexical fallback's 13.3 percent). The `ICandidateGenerator` seam remains for a future retrieval plugin. Behavior changes named: `FUSE_DENSE` is no longer read (retrieval is the lexical channel with the offline subword, stem, comment, centrality, and co-change signals); the on-disk index schema bumps 15 to 16 so a stale index carrying the `chunk_embeddings` table rebuilds itself without it on first touch; the `SignalGrader` insufficient floor was lowered from 0.30 to 0.20 so answerable-but-lexically-weak queries that used to clear the floor on a dense match are not falsely refused now the channel is gone (the genuine no-signal case is still caught upstream by `QuerySignalClassifier`). The opt-in ONNX reranker feature retired with the same project (it shared the model plumbing and was off by default, requiring an explicit `fuse models download`): `FUSE_RERANK`, `FUSE_RERANK_MODEL`, the `DenseRerank` experimental option, and the `fuse models` command are removed. Migration: reconnect or re-run any read tool; the index rebuilds on first touch. Docs and the localize and ranking result files are regenerated to the lexical-only numbers.
...
- **Parent consumes the build-capture worker across the process boundary (N4, tier-1, part 5).** Moved the capture DTO (`CaptureResult`, `CapturedProject`) and its source-generated JSON context into the shared `Fuse.Indexing` assembly, so the parent can deserialize the worker's output without referencing the worker's Basic.CompilerLog closure (which conflicts with the parent's MSBuildWorkspace). Added `BuildCaptureClient`: it spawns the worker (bounded args), reads the graph bundle from stdout, and deserializes it, degrading to a concrete failure (so the caller falls back a tier) when the worker is unconfigured, times out, or emits no parseable output. The worker is located via `FUSE_BUILD_CAPTURE_WORKER`. Validated end to end: the parent spawns the worker on a real project and reads back the extracted `Widget` symbol. Remaining: wire the client into the indexer as the tier-1 write path, and package the worker with the global tool.

- **Repair packets on `fuse_check` (R6, part 2 - R6 complete).** A speculative-typecheck diagnostic that is an API-shape error now carries fix context: CS1061 (missing member) returns the receiver type's real members with the nearest names first, and CS0246 (unknown type) returns the nearest type names, drawn from the persisted symbol table (new `GetMembersOfTypeAsync`) so the fix costs no extra round-trip. A packet is attached only where a concrete suggestion exists; on an unindexed type it explains the gap rather than guessing. No schema or protocol change. R6's turn-count benefit is measured by R4 (no standalone result file, per the plan).

- **The speculative staging area: `fuse_changeset` (M1).** A new MCP tool (the fourteenth) implements the propose-oracle-commit loop: create a changeset session, stage single-file edits in memory, diagnose them with the speculative typecheck (R1), select the tests that cover the changed symbols (R5's tests edges), then promote the diff to the working tree or discard it. Nothing is written until an explicit promote; two changesets over the same base are isolated. Backed by a new `ChangesetSessionStore`. No schema or protocol change (sessions are in-memory host state). The resident-workspace fast path for diagnose remains future work; in-process test execution is out of scope (M2).

- **The loop metric benchmark (R4).** A new eval suite (`fuse eval loop`, `results/loop.json`) measures build-gated turns to green per toolbox (native vs the fuse MCP tools), the metric the oracle thesis moves, which per-payload token counts do not capture. The deterministic core (`LoopTranscriptClassifier` plus `LoopMetrics`) is unit-tested and offline; a `fuse_check` turn counts as a build verification. The model-driven arms are opt-in (`FUSE_LOOP_RUN=1` with the claude CLI present); a bare invocation records the harness state and the task set it would sample without spending model time.

- **Covering-test selection (M1 down-payment).** `fuse_impact` now lists the tests that cover a symbol, selected through R5's DI-resolved `tests` edges and called out distinctly from the blast radius, so an agent can run just that subset with `dotnet test --filter`. The set is a lower bound bounded by R5 edge completeness, never presented as all the tests. New `GraphNeighborhoodExplorer.CoveringTestsAsync`; no new tool, no schema change. The full M1 changeset-session lifecycle remains future work.

- **Analyzer coverage table and contribution recipe (G2, docs).** A new docs page (`internals/extending/semantic-analyzer`) states exactly what the twelve shipped semantic analyzers cover (framework or pattern plus the edge or record each produces) and how to contribute one, so the coverage picture is honest and the on-ramp is documented. The semantic tier remains C#/Roslyn-only, stated plainly.

- **The ambient availability header (R3, part).** The store-backed oracle reads (`fuse_impact`, `fuse_signatures`) now open their response with a one-line availability header stating the index mode, whether tier-1 build capture is configured, and the freshness stamp, so a client cannot mistake a syntax-tier or stale answer for an oracle-grade one. The compiler tools (`fuse_check`, `fuse_refactor`) already carry their own explicit abstention line. No schema or protocol change. Remaining R3 work: the typed-union router that reshapes the read surface around the oracle.

- **Suite F: the `fuse_check` honesty gate (R1).** A new eval suite (`fuse eval checkgate`, `results/checkgate.json`) measures the two ways a speculative typecheck can lie: a false green (a broken edit reported clean) and a false red (a valid edit reported broken). It runs a battery of known-good and known-bad single-file edits through the shipped `CheckResult.IsClean` classification over a raw-Roslyn compilation (offline, no MSBuild). Current result: 8 of 8 edits classified correctly, zero false green, zero false red. The tier-1 worker path is covered by BuildCaptureCheckTests and recorded as skipped when no worker is provisioned rather than fabricating a number.

- **`fuse_refactor`: compiler-executed rename (R7, part 1).** A new MCP tool renames a symbol and every reference solution-wide through Roslyn's rename engine and returns the change as a staged diff, never touching the working tree; Roslyn semantics mean a same-named unrelated symbol is not renamed. It runs parent-side over an MSBuildWorkspace-loaded solution and abstains (with a reason) when the solution does not fully load, because a partial rename is worse than none. The MCP surface is now thirteen tools. Validated: renaming a fixture type stages a diff, and an unknown symbol abstains. Remaining R7 work: change-signature (the second operation) and re-checking the staged diff through fuse_check.

- **`fuse_check`: speculative typecheck (R1, part 1).** A new MCP tool typechecks a proposed single-file edit against the build-captured (tier-1) compilation and returns the compiler errors and warnings the change would produce, without writing the file or running `dotnet build`. The worker gains a `--check` mode: it builds and rehydrates, applies the patch to the compilation in memory (`ReplaceSyntaxTree`), and returns the changed document's diagnostics. `fuse_check` is oracle-grade: it answers only when the repo builds and the worker is configured, and abstains with a concrete reason otherwise, never guessing green. The MCP surface is now twelve tools. Validated: a patch that introduces a type error is reported, a clean patch is clean. Remaining R1 work: Suite F (agreement rate with `dotnet build`, false-green and false-red rates, both release-gated) and the resident-compilation fast path (the first cut re-captures per check).

- **Tier-1 build capture wired into the indexer (N4, tier-1, part 6).** `SemanticIndexer.IndexAsync` now attempts tier-1 build capture first when enabled (`FUSE_BUILD_CAPTURE` truthy and `FUSE_BUILD_CAPTURE_WORKER` set): it runs the worker via `BuildCaptureClient`, and on a successful capture writes the worker's graph bundle (symbols, nodes, edges, routes, DI registrations, options bindings) to the store, with the parent still producing files, syntax chunks, and embeddings. It falls back cleanly to the MSBuildWorkspace load (tier 2), then syntax (tier 3), on any capture failure. Default off, so indexing behavior is unchanged unless both env vars are set; the tier-1 data path is now functional end to end (worker builds and rehydrates and extracts, parent ingests). Remaining N4 work: package the worker with the global tool so the env var is not needed, add tier-2 auto-restore salvage, and run the localize/review recall re-run (the Phase 4 gate).

- **Build-capture worker emits the graph bundle (N4, tier-1, part 4).** The worker now carries the extracted graph (symbols, nodes, edges, routes, DI registrations, options bindings) in its result and serializes it as source-generated JSON on stdout, and a round-trip test pins the contract so the parent-side ingest reads exactly what the worker wrote. The remaining tier-1 work is the parent (indexer) spawning the worker, deserializing the bundle, and writing it to the store as the tier-1 path, plus packaging the worker with the global tool.
