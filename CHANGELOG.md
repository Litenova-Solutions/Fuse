...
- **Type-level refactors: extract-interface and move-type, verify-gated (T4).** `fuse_refactor` gains two more operations. `extract-interface` generates an interface from a class's public instance methods and properties (named with `newName`, else `I<Class>`) and makes the class implement it; `move-type` moves a top-level type to its own new file named after it, removing it from its current file. Both recompile the solution and return the staged change (full new file content) only when no new diagnostic is introduced, abstaining otherwise. New engine `TypeRefactorer` (in `Fuse.Semantics`). A precondition spike confirmed an `AdhocWorkspace` can host and run a `CodeFixProvider` on the `Microsoft.CodeAnalysis.Workspaces` abstraction alone (no `Features` package), so the remaining `apply-codefix` operation (drive an analyzer's own code fix across a scope) is viable and tracked as follow-up.
