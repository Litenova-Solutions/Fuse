...
- **Resident workspace foundation (S1, sub-step 1).** A new `Fuse.Workspace` core library introduces `ResidentWorkspace`: it rehydrates a tier-1 build capture (a binary log) once and holds the per-project Roslyn compilations in memory, so a proposed edit can be typechecked against a live compilation via an in-memory overlay (`CheckOverlay`) with no build and no disk write. This is the substrate the incremental watcher, delta check, and session overlays will stand on (replacing the rehydrate-then-discard pattern of the one-shot build-capture worker). The library references the `Basic.CompilerLog` rehydration closure and never `MSBuildWorkspace`, so a process hosting a resident workspace loads only the rehydration closure (Decisions D7/D8). This sub-step is additive and not yet wired into any tool surface (the serve/host wiring, the file watcher with incremental cone re-analysis, and the latency gate are the remaining S1 sub-steps); no shipped behavior changes yet.
