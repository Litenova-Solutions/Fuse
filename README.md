...
- **One scoped call instead of an explore loop.** Over 90 real merged pull requests, one `fuse --query` call delivers the files a change needs in about 37,000 tokens at 49 percent recall, against a generic-packer dump of about 425,000 tokens (about 11 times more) at the same single call, and about 409,000 tokens to read the repository blind. Fuse ties a packer at one call and wins on tokens; with a git base the change-scoping mode reaches 91 percent recall. Read the token number with its recall.
- **Cuts tokens without dropping API.** `--level none` removes 7 to 10 percent and `--level aggressive` removes 21 to 46 percent of tokens while keeping 99 to 100 percent of public types and methods. `--level skeleton` removes 39 to 56 percent for an architecture map at full signature fidelity.
...
- **Finds the files a change touches.** Change scoping recalls 89 percent of the files in 90 real merged pull requests at 54 percent precision, and all three scoping modes beat an agent-style grep baseline.
- **Trustworthy skeletons on hard code.** Roslyn structural analysis keeps 100 percent of method signatures on all five benchmark libraries, including Newtonsoft.Json, where the retired regex skeleton kept 4 percent.
