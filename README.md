...
- **Verifies an edit without lying.** On the hand-built check-honesty set, `fuse_check` classified all 8 cases correctly with zero false green and zero false red: a compile-breaking edit is called broken, a clean edit is called clean, and when the substrate cannot answer at compiler grade the tool abstains rather than guess. (This program scales that measure to 1,000-plus mutation-derived cases; see the roadmap.)
