...
Remaining work on the built layers: a corpus arm that runs the precision tier (so the Roslyn skeleton gain over the regex collapse is measured, not only unit-tested), and a TypeScript or SQL repository in the corpus to confirm a cross-language reduction ratio.
