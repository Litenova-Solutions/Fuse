...
### R8 Peer comparison and agent suite at scale (2026-06-26, blocked)

- Status: Blocked on external resources, box unticked. The agent suite (Suite D) and the layer-6 peer harness exist from prior work; what R8 adds is running them at 50 to 100 tasks by 3 rollouts in semantic mode with hybrid retrieval against CodeGraph, Serena, and coa-codesearch.
- Blockers (concrete): (1) the peer tools (CodeGraph, Serena, coa-codesearch) are not installed on this branch, so a head-to-head needs an external install step; (2) 50 to 100 tasks by 3 rollouts by two arms is 300 to 600 real Claude Code CLI rollouts, hours of model compute, which is an explicit budget commitment, not an in-loop run; (3) the at-scale agent and peer numbers are most meaningful in semantic mode, which is bounded by R0 (only two of six repos restore on this SDK). The existing `agent.json` (6 PRs by 1 rollout) is the recorded small-sample baseline. Unblocking needs peer-tool installation, a compute budget, and corpus re-pinning so more repositories restore.
- Surfaced: this item is left for a run with the resources provisioned; the engine and harness it depends on are in place (Suite D driver, the dense/hybrid retrieval from R1 and R2, the restore path from R0).
