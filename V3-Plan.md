...
### R5 Wider semantic analyzer set (2026-06-26, partial)

- Status: In progress. Three new analyzers landed and measured; open and closed generic DI was already covered; minimal-API route groups, gRPC and SignalR endpoints, and decorator and factory-lambda registrations remain. The box stays unticked until the remaining kinds land.
...
- Blockers: None; the remaining wiring kinds (minimal-API route groups via `MapGroup`/`MapGet`, gRPC via `MapGrpcService<T>`, SignalR via `MapHub<T>`, Scrutor `Decorate<,>`, and factory-lambda `new T()` edges) are the same invocation-scanning pattern and are follow-up, not a blocker.
- Lessons: Compiling the fixture in-memory for unit tests (`OrderingAppFixture.Load`) means a new wiring kind needs only a stub type in `Framework.cs` and a source file, with no MSBuild restore, so analyzers are cheap to test; Suite A then exercises the same fixture through the real MSBuild load. Synthetic named-contract endpoints (a `service:` node) keep an edge's endpoints concrete when the "from" side is a framework interface, which the every-endpoint-has-a-node invariant requires.
- Time: about 1 session.
