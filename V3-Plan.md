...
### R6 Suite A on the wiring zoo (2026-06-26, partial)

- Status: In progress. The fixture catalog is expanded to the Section 18.3 edge-case set and Suite A scores it at 22 of 22 edges (100 percent recall and precision); the sampled adjudication of predicted edges over the OSS corpus remains. The box stays unticked until corpus adjudication lands.
...
- Blockers: The corpus-wide sampled adjudication (sample N predicted edges per type over the OSS repos, label them, report precision with confidence intervals) is bounded here by two things: the corpus mostly loads in syntax mode (R0: only two of six repos restore on this SDK, and syntax mode emits no graph edges to sample), and labeling sampled edges needs a human or strong-model adjudicator. The fixture catalog (the deterministic moat) is complete; the corpus precision number is the remaining, environment-bounded piece.
...
### R7 Warm host and latency suite (2026-06-26, partial)

- Status: In progress. A `performance` suite measures cold index and warm operation latency; the warm numbers meet the crown targets. Single-file incremental re-index and the review-plan latency remain, and the warm host is not yet the measured runtime. The box stays unticked until the incremental path and host-driven timing land.
- Result: Added `PerformanceSuite` (`fuse eval performance`, `performance.json`) that indexes a present corpus repository once and then runs resolve and localize repeatedly over the warm index, reporting P50/P95 via a tested percentile helper. Measured on AutoMapper (518 files, 17,924 symbols, syntax mode): warm resolve sub-millisecond, warm localize 20 ms P50 / 22 ms P95, both inside the targets (resolve under 100 ms, localize under 500 ms); cold index about 65 seconds including the MSBuild load attempt. Corrected the performance docs, which had claimed incremental re-index was implemented when a re-index in fact runs a full pass.
- Verification: `dotnet build` 0 errors; `dotnet test` all 15 projects green (Fuse.Benchmarks.Tests 27 -> 33: percentile helper edge cases and interpolation). `dotnet format` clean. `fuse eval performance` records the latencies.
- Blockers: None for the measured part. Remaining: a single-file incremental re-index path is engine work (semantic edges are cross-file, so single-file invalidation needs a careful design of which edges to recompute), and the review-plan latency needs a git base; driving resolve/localize/review through the warm host process (rather than a fresh in-process store) is the host-runtime piece.
- Lessons: A re-index currently runs a full pass; the doc claim that a changed file is re-indexed incrementally was aspirational and was corrected. Cold index time is dominated by the MSBuild workspace load attempt even when the result falls back to syntax mode, so cold and warm are very different regimes and only warm is the steady state.
- Time: about 1 session.
