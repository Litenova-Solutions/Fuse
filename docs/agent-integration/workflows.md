...
1. Call `fuse_skeleton` first. It returns signatures without method bodies, which produces a low-token architecture map of the whole codebase. The agent reads the manifest and the skeleton to learn which projects and types exist.
...
3. For pull-request review, call `fuse_changes` with the base branch as the git ref. It returns the files changed since that ref plus their first-degree dependents.
