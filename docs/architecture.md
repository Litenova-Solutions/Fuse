...
Fuse.Emission       -> Fuse.Reduction
Fuse.Fusion         -> Fuse.Collection, Fuse.Reduction, Fuse.Emission
...
### ReductionOptions (10 properties)
...
The `--all` flag sets each individual C# reduction property to `true` at the CLI boundary. No reducer checks an `ApplyAllOptions` flag.
...
FileCollectionPipeline  (enumerate candidates, apply filters)
    |
    v
ContentReductionPipeline  (read, normalize, reduce, filter trivial)
    |
    v
