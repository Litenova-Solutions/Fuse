...
Reduction results are cached per file under `.fuse/cache`, so an unchanged file is not reduced again on a later run. The cache is on by default; disable it with `--no-cache` and clear it with `--clear-cache`. The cache key and eviction behavior are documented in [Caching Internals](../architecture/caching-internals.md).
