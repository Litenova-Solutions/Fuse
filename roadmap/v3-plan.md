...
The first V3 wave shipped a Roslyn-backed .NET semantic context engine with a warm persistent index, an 8-tool MCP surface, a five-suite C# benchmark, and a rewritten docs site. Its plan, execution record, and full progress log are archived in the V3 overhaul history at the end of this file (and PR #20).
...
- Blockers (concrete): the test oracle needs the corpus repository to build and run its tests, and the pinned corpus does not build on the installed SDK. Confirmed by attempting `dotnet build src/Serilog/Serilog.csproj`, which fails fast with NU1008 (central package management version conflict, the same pinned-era SDK drift R0 surfaced for restore), so there is no green build for an oracle to run against. Separately, the at-scale run drives a real agent per task across two arms to produce the patch, which is hours of model compute and needs a budget. The deterministic core is complete and trusted; the prize number needs both a buildable, testable corpus (corpus re-pinning) and the compute.
...
# Archived: V3 overhaul history

The V3 overhaul plan, design spec, and full execution record below predate the V3 forward plan at the top of this file. It is kept here for history; every box is ticked (see PR #20).

# Fuse V3 Overhaul Plan (archived)

> Archived execution record. The V3 overhaul described here is complete (every box below is ticked; see PR #20). This file is kept for history: the original plan, the design spec, and the full progress log. The forward-looking plan is at the top of this file.
...
- Suite C demonstration (real, not committed to results because it needs the corpus): cloned MediatR at its pinned commit, indexed it (MSBuild .slnx load failed -> syntax fallback worked on a real repo: 168 files, 1403 symbols), reconstructed 10 real merged-PR change sets from merge history, and ran `fuse localize --task "{PR title}"` measuring recall of the PR's changed .cs files. Mean localize recall 0.56 over 10 PRs, with the predicted shape: identifier-rich titles 1.00, no-signal titles ("Apply suggestions from code review", "Merge branch master into master") 0.00. This matches the recorded query-floor order and confirms the suites are runnable end to end; the full 6-repo/108-PR corpus run plus the Claude Code CLI agent arm (Suite D) is the remaining large compute.
- Time: ~60 min

### P10.3 benchmark rewrite in C# (Fuse.Benchmarks project) - 2026-06-26 (continued)
- Status: in progress (suites A-D coded and green; corpus runs executing)
- Result: Replaced the per-suite PowerShell harness for the new suites with a first-class C# project `tests/benchmarks/Fuse.Benchmarks` (+ `Fuse.Benchmarks.Tests`), wired into `Fuse.slnx`; the `fuse eval` CLI is now a thin entry that delegates to it. Pieces landed in order:
  - Scaffold (commit 09a0713): typed model (`EvalDataset`/`RepoTasks`/`PrTask`/`GroundTruth`/`SuiteResult`/`Scorecard`/`TaskResult`/`CorpusManifest`/`PrRecord`), source-generated `BenchmarkJsonContext` (reflection-free invariant honored), `Metrics` (recall/precision/F1/median/mean + deterministic seeded bootstrap CI), `SignalBucket` classifier (no-signal/dependency-bump/config-ci/formatting/route-api/test-only/identifier-rich/nl-domain), `IEvalSuite`/`EvalOptions`. +20 tests.
  - Suite A (commit c0e13f7): `SemanticResolutionSuite` ports the proven edge-gold scoring; `fuse eval semantics` reproduces 100/100 on OrderingApp. Removed the old `EvalResultsDto`.
  - CorpusManager (commit c87e59d): `corpus.json`/`prs.json` loading into `EvalDataset`, `gen-prs.ps1` C# port (`ReconstructPullRequestsAsync`), git worktree helpers, `GitCli` (bounded args). +4 tests on a tiny in-test git repo and the real manifest/prs (24 total in the bench test project).
  - Suite B (commit de4eeeb): `ChangeImpactSuite` worktrees each PR head, indexes it, runs `fuse review` against the base, scores changed-file recall/precision/tokens, and records the index-mode distribution.
  - Suite C (`LocalizationSuite`) and Suite D (`AgentSuite`, native+fuse arms driving the `claude` CLI with stream-json parsing) coded; library builds clean (0 warnings).
- Verification: `dotnet build Fuse.slnx -c Release` green; bench tests 24 passed; `dotnet format --verify-no-changes` clean. `fuse eval semantics` 100/100; `fuse eval review` smoke on MediatR/eShopOnWeb returns real numbers. Full corpus runs (review/localize) and a Suite D sample are executing to produce committed `results/*.json`.
- Blockers/issues: Corpus repos load in syntax-fallback mode here (MSBuild semantic load needs a restored workspace), so Suite B review scopes to changed files (recall/precision ~1.0, low tokens); recorded honestly with the index-mode note. Semantic blast radius is the Suite A measurement (edge gold 100%).
