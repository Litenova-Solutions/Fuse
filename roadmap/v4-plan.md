...
- [x] G5 Shared resident daemon: one process per root serving MCP, host, hooks (depends: S1, S3) (2026-07-10 GATE MET: one-truth test green (DaemonOneTruthTests: two pipe clients reading one daemon see the same CS0246 from its single resident workspace) + RSS reduction measured on NodaTime (one daemon 109 MB vs the two-process baseline 218 MB, 50 percent, because a second `fuse host` exits under the single-instance DaemonLock). Shipped opt-in (FUSE_DAEMON delegates serve's resident checks to a shared daemon over the pipe with the in-process fallback preserved); the single-instance lock + idle shutdown (FUSE_DAEMON_IDLE_MINUTES) + the ProtocolVersion 7 version-handshake are always on for `fuse host`. Surface: DaemonLock, DaemonSupervisor, IsServingAsync/TryStatsAsync/fuse-checkOverlay probes, RemoteResidentWorkspaceProvider proxy, DaemonProcessLauncher, IdleShutdownMonitor, `fuse workspace status` daemon line; docs internals/daemon.mdx + config keys + CHANGELOG. See the 2026-07-10 G5 GATE MET progress-log verdict.)
...
#### 2026-07-10 G4 sub-steps 4-G2+4-G3 DONE -> G4 [x]: Fuse.Capture target package (experimental) + docs + Gate

**Shipped (4-G2).** `src/Packaging/Fuse.Capture/` - a build-logic-only NuGet package (`Fuse.Capture`,
netstandard2.0, no assembly) shipping `build/Fuse.Capture.targets`. The target `FuseEmitCaptureFragment`
(AfterTargets=Build) emits a per-project fragment binlog into a shared `FuseCaptureFragmentsDir` by a
nested non-incremental build guarded against recursion by `-p:_FuseCaptureInner=true`; opt out with
`FuseCaptureEnabled=false`; ContinueOnError so a capture failure never breaks the developer's build.
Added to Fuse.slnx. Packs with `build/Fuse.Capture.targets` in the nupkg (verified), so a
PackageReference imports it.

**Validated end-to-end (real builds).** A fixture importing the targets: baseline build 1281 ms;
with-capture 2445 ms; the fragment `Gizmo.binlog` (418 KB) was emitted; `fuse capture --merge` then
assembled a format-v2 bundle from it (1 project). Overhead ~90% of build time.

**Docs (4-G3).** capture-bundles.mdx gained a channel-comparison table (Action / direct / build-target
package) with the invasiveness + ~90% overhead note; the .targets header says EXPERIMENTAL.

**Tests.** `FuseCaptureTargetsTests` parses the shipped .targets as XML (guards the "--"-in-comment
trap that silently breaks the import - which bit twice this session) and asserts the emit target,
the opt-out property, the recursion guard, and AfterTargets=Build. `CaptureFragmentMergeTests`
(4-G1a) + `CaptureBundleV2Tests` (4-G1b) cover the merge engine and v2 layout.

**Gate.** "Merge-equality green" -> MET (4-G1a edge-set equality proof + 4-G1b end-to-end). "Overhead
under 5% of fixture build time" -> NOT met (~90%, a nested recompile) -> the item's named Fallback
applies: "ship as experimental with the overhead published" - done (the .targets and docs both say
experimental with the ~90% figure). Gate satisfied via the Fallback. G4 [x].

**Note.** The `Fuse.Capture` package builds and packs locally; publishing it to NuGet on a release is
a [maintainer] decision (publish.yml currently ships the `Fuse` tool only).
...
#### 2026-07-10 G5 GATE MET -> G5 [x]: shared daemon, one-truth green + RSS reduction measured

**Sub-step D (the Gate).** One-truth test: `DaemonOneTruthTests.Two_clients_reading_one_daemon_see_one_
truth` spawns a real `fuse host` daemon on a fixture with FUSE_RESIDENT, waits for its resident
workspace, then two independent pipe clients check the same broken edit and BOTH get the same CS0246
from the one daemon-held workspace - green (ran in 9 s, not the abstain path). RSS reduction (measured,
NodaTime): one daemon holds the resident workspace at 109 MB; a second `fuse host` on the same root
EXITS under the single-instance lock ("a daemon already serves ...; not starting a second"), so the
two-process baseline (2 x 109 = 218 MB) collapses to 109 MB - a 50 percent reduction, recorded in
performance.json.

**Docs.** internals/daemon.mdx (what runs, how to see it via `fuse workspace status`, how to stop it -
idle shutdown, version handshake, manual); configuration-keys.mdx (FUSE_DAEMON, FUSE_DAEMON_IDLE_
MINUTES); CHANGELOG "The shared daemon" bullet (behavior: opt-in daemon delegation).

**Gate verdict.** "One-truth test green" -> MET (DaemonOneTruthTests passes). "RSS reduction versus the
two-process baseline recorded" -> MET (109 MB vs 218 MB, 50 percent, performance.json). G5 [x].

**Full G5 surface shipped (all committed + tested):** DaemonLock single-instance (181cda7), host
acquires it (80eeab9), DaemonSupervisor spawn-on-demand + IsServingAsync (8d93269), IdleShutdownMonitor
(6abf948), status line + TryStatsAsync (fa54c7e), fuse/checkOverlay RPC ProtocolVersion 7 (ae1798c),
RemoteResidentWorkspaceProvider (6c1e141), DaemonProcessLauncher + serve FUSE_DAEMON delegation
(44472c8), plus the one-truth test + RSS + docs (this commit). Shipped opt-in (FUSE_DAEMON) with the
single-process fallback preserved; the daemon single-instance lock and idle shutdown are always on for
`fuse host`.

**Pre-existing flake noted (not G5):** Fuse.Fusion.Tests has one timing-flaky test (1/144 intermittent;
passes on re-run), unrelated to the Fuse.Cli G5 changes. Recorded for a follow-up hardening; not a G5
regression.

**Next action.** R3 (Release hygiene and the v4 cut; depends R1, R2 both [x]): canonical results regen
(including the tier-1-on ranking shift observed under C3: recall@10 12.6 percent vs the recorded 15.0
percent, within CI - regenerate ranking.json/review.json under shipping defaults and sweep the
superseded doc figures in the same change), assets, briefing refresh, release prep. Tag and publish
are [maintainer].
...
#### 2026-07-10 Runway terminal state: no eligible unblocked item remains

Full Master checklist state after this session's work. Done [x]: R1, R2, C1, C2, G4, G5 (plus the
prior S/T/U/K/H/G3/G8/B2/F3/X1 waves). Blocked [!], each with its engine or prep complete and its gate
provisioning/compute/corpus/maintainer-bound:
- C3 [!] tier-1 default-on: engine complete + committed (two real bug fixes landed); localize main
  3/4 semantic and ranking within CI MET; review-semantic gate needs C4's buildable corpus.
- C4 [!] corpus v2: engine complete (corpus-health suite, TaskOracle verifier, model-suite refusal
  gate) + committed; the 20-repo/60-task curation is compute-bound (needs a provisioned runner).
- G2 [!] analyzer pack: iteration 1 gate-green (moat 23/23); iteration 2 needs C4 corpus-v2 frequency
  data.
- R3 [!] release cut: version scripts + site build + briefing prose done; canonical benchmark-numbers
  regen needs C4; merge/tag/publish are [maintainer].

Not startable [ ] (dependency is [!] or not done, per the selection rule; NOT eligible):
- B1 (depends C4 [!]) -> blocked; B4 (depends C3 [!]) -> blocked; B3 (depends B1 + [maintainer publish])
  -> blocked; F2 (depends B1) -> blocked.

Every remaining item is done, or [!]-blocked on something only the user can supply: a provisioned
build/test/model runner (C4's 20/60 corpus and B1's model suite), the buildable corpus-v2 that C3's
review gate and R3's canonical numbers and G2's frequency data all wait on, or the [maintainer] tag.
No eligible, unblocked checklist item remains, so the selection rule yields nothing further to start
in this environment. Per D17/D18 the corpus and model runs are the provisioning the goal authorizes
the user to supply; when a provisioned runner exists, the exact next actions are recorded in each
