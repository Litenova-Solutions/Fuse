...
- [ ] T4b apply-codefix: drive the repo's analyzer code fixes across a scope (depends: T4) (split from T4 2026-07-08: the codefix-hosting spike is RESOLVED and proven (CodeFixHostingSpikeTests), so this is viable; it loads [ExportCodeFixProvider] types from the compilation's analyzer references by reflection, runs analyzers for the target diagnostic id, drives the FixAll/one-by-one hosting loop, and verify-gates that the target diagnostic count goes to zero with no new errors - the careful analyzer-execution + reflection + FixAll unit deferred from T4 rather than rushed)
