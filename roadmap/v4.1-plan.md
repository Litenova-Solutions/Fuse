...
- [>] G8 CI parity rehearsal (depends: T0) (2026-07-08: precondition spike done on the corpus - all 4 repos have .github/workflows; dotnet commands ARE best-effort extractable: eShopOnWeb (`dotnet build/test ./eShopOnWeb.sln`), Specification (`dotnet build/test ci.slnf`), Scrutor (`dotnet tool install`, `dotnet pack`, `dotnet nuget push`) are direct `run: dotnet ...` steps; NodaTime uses a `run: |` multi-line block (parse the block body). Non-rehearsable classes identifiable: secrets (nuget push GITHUB_TOKEN), setup-dotnet matrix TFM legs, service containers. Hit rate is good; the escape hatch (--commands) covers the tail. Next: implement `fuse verify --ci-parity` (parse workflows -> extract dotnet step sequence -> run via T0's executor -> gap report classifying non-rehearsable steps; no silent skips))
