...
- [>] T3 Constrained change-signature, verify-gated (depends: S1, S2, T1) (2026-07-08: preconditions recorded (rename pipeline = RenameRefactorer, SymbolFinder available in the referenced Microsoft.CodeAnalysis.CSharp.Workspaces); building the add-parameter core first (the flagship + the item's Fallback floor), verify-gated by recompile-and-diff-diagnostics)
