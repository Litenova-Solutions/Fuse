...
- [>] F3 NuGet upgrade oracle: package-bump break prediction (depends: T2) (2026-07-08: precondition spike done - (a) version PAIRS are cached locally so an offline diff is possible (newtonsoft.json 13.0.1/13.0.3/13.0.4, System.Text.Json 6.0.10/7.0.0, Microsoft.Extensions.DependencyInjection.Abstractions 6.0.0/10.0.x); network is only needed for an un-cached target, where the oracle abstains with a reason. (b) T2's PublicSurfaceExtractor is SYNTAX-based (CSharpSyntaxTree.ParseText), so F3 needs a NEW metadata-surface extractor (load the DLL as a MetadataReference -> walk IAssemblySymbol public types/members); T2's PublicApiDelta comparison logic is REUSABLE once two surfaces are produced. Sub-step plan: (1) MetadataSurfaceExtractor DONE, (2) PackageUpgradeOracle.Analyze(id, referencedDll, targetDll) DONE - break set via the reused PublicApiDelta, offline abstain when a version DLL is absent, blind spots (reflection/dynamic, external call-site coverage) named on every report; 7 tests. REMAINING: (3) call-site intersection is BOUNDED by R5 (references edges are FK-safe to source types only, so external-package call sites are not tracked - the item's Fallback applies: ship the API-delta half and say so, which the oracle's BlindSpots note does), (4) `fuse_impact package:{id,toVersion}` wiring (resolve the referenced version DLL from the repo's restore + the target from the cache, abstain offline) + a curated 3-known-breaking-upgrade fixture + the zero-false-safe Gate. NEXT ACTION: F3 sub-step 4 (fuse_impact package: wiring + DLL resolution + curated fixture). M, offline-doable for cached pairs)
