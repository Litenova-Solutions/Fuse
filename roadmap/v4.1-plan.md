...
loader") as the first committed sub-step behind the chosen option, with the issue-5 acceptance test (an
edited DI registration queryable without a full IndexAsync) written first; (3) proceed through the item's
steps 2-6, landing each as a green sub-step. Do NOT keep MSBuildWorkspace resident (item Do-not); do NOT let
the watcher write the tree.
