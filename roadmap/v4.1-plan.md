...
- [>] G2 Analyzer pack: third-party framework coverage and the community on-ramp (depends: S1) (2026-07-08: precondition investigated - the ISemanticAnalyzer seam + DiRegistrationAnalyzer extraction analyzed; keyed DI confirmed a blind spot (the method-name map has AddScoped/Singleton/Transient + TryAdd only, no AddKeyed*). First iteration = keyed DI: the two-type-arg keyed generic and the typeof-keyed forms extract correctly with just method-name additions (the typeof filter drops the key arg), but the single-type-arg keyed form needs the key-vs-factory disambiguation; the analyzer change is precision-critical (Suite A must stay 1.0) so it is a careful fresh-context sub-step, not a depth-rushed one. Framework-frequency selection for later iterations is gated on corpus-v2 (C4))
