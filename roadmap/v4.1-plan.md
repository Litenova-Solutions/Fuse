...
- [>] F3 NuGet upgrade oracle: package-bump break prediction (depends: T2) (2026-07-08: precondition spike done - (a) version PAIRS are cached locally so an offline diff is possible (newtonsoft.json 13.0.1/13.0.3/13.0.4, System.Text.Json 6.0.10/7.0.0, Microsoft.Extensions.DependencyInjection.Abstractions 6.0.0/10.0.x); network is only needed for an un-cached target, where the oracle abstains with a reason. (b) T2's PublicSurfaceExtractor is SYNTAX-based (CSharpSyntaxTree.ParseText), so F3 needs a NEW metadata-surface extractor (load the DLL as a MetadataReference -> walk IAssemblySymbol public types/members); T2's PublicApiDelta comparison logic is REUSABLE once two surfaces are produced. Sub-step plan: (1) MetadataSurfaceExtractor (testable against a cached assembly), (2) diff two versions via PublicApiDelta, (3) intersect with R5 references edges for call-site evidence, (4) `fuse_impact package:{id,toVersion}` wiring + a curated 3-known-breaking-upgrade fixture + the zero-false-safe Gate. M, offline-doable for cached pairs)
