...
- [>] C1 `fuse up`: the environment remediation engine (depends: X1) (2026-07-08: preconditions recorded; sub-step 1 landed - RemediationKnowledgeBase (JSON-data KB + source-gen loader + per-signature matcher, classify-only), gate-green; the fuse up command that applies remedies and the 17-repo up-report.json gate are pending)
