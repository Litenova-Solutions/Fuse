...
tier-1 bounded to two rounds; the report to workspace status (U1); the KB-generated troubleshooting page with a
key-diff test; and the 17-repo `up-report.json` Validation/Gate (likely landing on the Fallback here, "1 of 6
flips" via NU1507 only, unless installs are provisioned).
