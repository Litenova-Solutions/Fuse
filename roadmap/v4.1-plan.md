...
- [>] S1 The resident workspace: compilation as source of truth (depends: X1) (2026-07-08: preconditions verified with file:line and the resident-compilation architecture decision recorded; sub-step 1 implementation pending a dedicated session)
...
- [ ] C1 `fuse up`: the environment remediation engine (depends: X1)
...
Next session: (1) run the closure-coexistence spike and record which option (a/b/c) survives; (2) implement
S1 step 1 ("extract a rehydration-to-resident loader") as the first committed sub-step behind the chosen
option, with the issue-5 acceptance test (an edited DI registration queryable without a full IndexAsync)
written first; (3) proceed through the item's steps 2-6, landing each as a green sub-step. Do NOT keep
MSBuildWorkspace resident (item Do-not); do NOT let the watcher write the tree.

### 2026-07-08 C1 advance scouting: bake-off failure classification (C1 stays [ ])
Read-only preparatory analysis so whichever session takes the Wave 2 independent lane can start C1 without
re-scouting. C1 is NOT started (stays `[ ]`); S1 remains the single in-progress keystone. Source:
...
Doctor's downgrade-reason strings are the match surface and must be confirmed against `fuse doctor` output at
implementation time (not re-run here). Full C1 preconditions (doctor reason strings confirmed live) are to be
recorded under a proper C1 `[>]` entry when the item is started.
