...
- [>] T4 Refactor family expansion: move-type, extract-interface, codefix-apply
      (depends: S1, S2, T3) (2026-07-08: precondition spike RESOLVED - an AdhocWorkspace hosts and applies a CodeFixProvider using only Microsoft.CodeAnalysis.Workspaces (no Features package), proven by CodeFixHostingSpikeTests; so codefix-apply is VIABLE, not descoped. Next sub-steps: move-type + extract-interface (structural rewrites like T3, verify-gated) then apply-codefix (load [ExportCodeFixProvider] types from the compilation's analyzer references by reflection and drive them through the proven hosting loop))
