...
- [ ] X1 Execution contract into AGENTS.md; identity rewrite (depends: -)
- [ ] K1 Retire the dense embedding channel (depends: -)
- [ ] K2 Delete the in-memory BM25F ranker; host scoping on the engine (depends: -)
- [ ] K3 Formal closures: V1/V2, provider freeze, Suite D retirement (depends: -)

Wave 1: resident substrate and honesty floor
- [ ] S1 The resident workspace: compilation as source of truth (depends: X1)
- [ ] S2 Delta check, persisted sessions, repair packets v2 (depends: S1)
- [ ] H1 Mutation-derived honesty calibration at scale (depends: -; re-run after S2)
- [ ] T0 Verification-grade ladder: build-grade fallback, verify never shrugs (depends: H1)
...
(Empty at program creation. One entry per item, per the execution protocol: preconditions
verified with file and line references, what shipped, commands run with pasted output,
numbers recorded and where, deviations, gate verdict.)
