...
- [ ] U1b fuse_find kind=signatures over referenced-assembly metadata when resident (depends: U1) (split from U1 2026-07-08: the signatures mode ships store-based; answering over a referenced package's metadata assemblies when a resident workspace serves the root - the hallucinated-package-API killer - is the named U1 Ships refinement, deferred; F3's MetadataSurfaceExtractor already extracts a DLL's public surface, so this wires that into fuse_find kind=signatures behind the resident check)
