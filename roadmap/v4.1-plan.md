...
- [>] S2 Delta check, persisted sessions, repair packets v2 (depends: S1) (2026-07-08: delta mode wired into fuse_check (session+full+markGreen params, resident whole-state diagnostics via a new TryGetCurrentDiagnostics seam, DiagnosticDelta introduced/resolved, repair packets on introduced), sessions persist to the store (additive check_sessions table, restart-resumable), repair packets expanded to CS7036/CS0029; docs + CHANGELOG swept; all gates green. Remaining: the delta-p95 latency re-measurement through the wired tool (needs a resident + tier-1 environment))
