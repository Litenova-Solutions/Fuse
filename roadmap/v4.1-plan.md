...
- [>] S3 Harness hooks: ambient verification (depends: S2) (2026-07-08: preconditions recorded with file:line; the protocol-bump + pipe-RPC core is a cross-cutting shipped-path change to start fresh next session)
...
- [>] T1 Covering-test execution out of process (depends: S1, S2, H1) (2026-07-08: build-grade floor SHIPPED both surfaces (fuse_test MCP + fuse test CLI): select covering test types -> dotnet test --filter -> per-test verdicts + not-runnable classification, end-to-end tested; the emit fast path is deferred (compile-time refs cannot run; needs the runtime closure - the published Fallback miss); remaining: the testexec.json benchmark suite (selection-safety >=95%, median latency) + the H1 behavior-mutant extension)
