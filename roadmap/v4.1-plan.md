...
- [>] T4b apply-codefix: drive the repo's analyzer code fixes across a scope (depends: T4) (2026-07-08: CORE shipped - CodeFixApplier.ApplyAsync drives the run-analyzer -> apply-fix-per-occurrence -> re-analyze -> verify loop, bounded, verify-gated (target id to zero, no new error, else abstain); 3 deterministic tests with an in-test analyzer+fix. Remaining: (2) reflection-load the analyzers + [ExportCodeFixProvider] fix providers from the project's AnalyzerReferences, (3) a public ApplyCodeFixAsync(path, diagnosticId, file) that loads the solution and discovers them, wired into fuse_refactor (operation=apply-codefix), (4) fixture test over a real analyzer reference + docs + Gate)
