...
- [ ] T3b change-signature: remove-parameter (unused-everywhere else abstain) + reorder (named-arg sites only) (depends: T3) (split from T3 2026-07-08: the add-parameter core + CancellationToken recipe shipped and gate-green; remove/reorder are the lower-value remaining operations, reorder carrying the item's stated silent-semantic-change kill risk so it is gated to named-argument call sites only)
