...
- [>] C1 `fuse up`: the environment remediation engine (depends: X1) (2026-07-08: preconditions recorded; sub-steps landed - RemediationKnowledgeBase (JSON-data KB + matcher) and EnvironmentRemediationPlanner (classify-and-report core: per-project remedy classification + workable-subset line), both gate-green and non-user-facing; the fuse up command that applies remedies and the 17-repo up-report.json gate are pending)
...
86 -> 87. Gates: build 0 errors; all 16 test assemblies pass; format exit 0. This is the availability half of
step 2; the "route a read tool's ANSWER through resident-first" half and the serve/host wiring that sets a real
provider remain (the watcher sub-step below and the serve integration).
