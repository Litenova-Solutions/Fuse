...
description: Every statement Fuse emits in an answer is graded by the evidence behind it - verified, partially verified, stale, or contradicted - so an agent and its human know which claims are compiler-grade truth and which are weaker inferences that can go stale.
---
...
resolved wiring edge, a review summary, a covering-test run. Each such statement carries a
**claim grade** computed from the evidence available, so a reader always knows which parts of
an answer are compiler-grade truth and which are graph-grade inferences that may not hold.
...
- `fuse_review` - the changed-file set (git-truth, `verified`) and whether the change alters
  the public API surface (graph-grade).
...
handoff **refuses** and returns the red summary instead of a packet. Fuse gates the change; it
does not commit for you. This refusal is the gate-not-controller stance expressed as one
behavior - you cannot hand off a change that does not yet pass the compiler.
