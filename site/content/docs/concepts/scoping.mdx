...
hits. When a local embedding model is present and dense indexing is enabled (`FUSE_DENSE`),
an optional dense channel ranks files by embedding similarity, so a prose task that shares
no words with the code still finds the right file; with no model the path stays lexical and
offline. Feed the candidate file paths to `fuse_context` to fetch their bodies. It
