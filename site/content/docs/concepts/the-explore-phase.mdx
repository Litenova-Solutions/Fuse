...
```mermaid
flowchart LR
    A[Survey<br/>fuse_map] --> B[Localize<br/>fuse_localize]
    B --> C[Resolve<br/>fuse_resolve]
    C --> D[Context<br/>fuse_context]
    D --> E[Review<br/>fuse_review]
    E -.next step.-> B
```

Survey the workspace cheaply with `fuse_map`, localize a task to candidate files with
`fuse_localize`, resolve the wiring a task names with `fuse_resolve`, plan and emit the
context with `fuse_context`, and review a branch with `fuse_review`. Pass a `sessionId`
...
open-ended, start with `fuse_localize`; when it names a route, interface, request, or
config section, start with `fuse_resolve`.
