...
The [Layer 4 benchmark](/docs/project/benchmarks) puts numbers on the loop over 24 real
...
context. That is a structural lower bound, not a measured agent: a mean of 5.8 files per
change, from 2.3 on AutoMapper to 14.5 on Newtonsoft.Json. Reading the repository blind to
find them averages about 493,661 tokens; the files that actually matter are about 27,473
tokens of that.

## The Collapse: One Scoped Call

Fuse acquires that context in one call. Read its token number with its recall: one scoped
`fuse --query` call delivers about 40,041 tokens at 51 percent recall of the changed
files, and the change-scoping mode reaches 88 percent recall when a git base is available.

Against a generic packer (Repomix), which dumps the whole repository in one call of about
511,574 tokens, Fuse ties on round-trips and wins on tokens, roughly 12.8 times fewer,
