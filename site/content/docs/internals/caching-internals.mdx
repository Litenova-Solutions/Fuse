...
Each file row stores an XXHash64 of its content. A warm reconcile scans the complete current inventory. An unchanged hash is left as is; an added or edited file is extracted; a removed file is hard-deleted from `files`, its derived tables, FTS, and co-change rows. The directory scan discovers additions without an explicit full index, while extraction remains proportional to the files whose content changed.
