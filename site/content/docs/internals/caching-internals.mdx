...
## The Persisted Relevance Index

Query scoping builds a BM25 index over the candidate files each call, and body tokenization is the dominant cost of that build. When the persistent index is enabled, the body tokens of each file are cached on disk under `.fuse/index` keyed by content hash, alongside the analysis index. A warm query against an unchanged tree finds every file's tokens in the cache and skips re-tokenizing, so the index build drops from a cost in the number of files to a cost in the number of changed files plus the query. Invalidation is automatic: an edited file hashes differently, misses the cache, and is re-tokenized, while its neighbours still hit. Only the body field is persisted; symbol and path tokenization are cheap, and symbol analysis is already served by the analysis index. The cache is best-effort and content-addressed, so it is safe to share across concurrent runs and a corrupted entry degrades to recomputation. Ranking with the persisted tokens is identical to the in-memory build.
