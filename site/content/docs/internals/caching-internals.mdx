...
All persistent cache data lives in a single SQLite database at `.fuse/fuse.db` under the source root. The file uses WAL journal mode and a single `kv` table partitioned by a `store` column. Four logical namespaces share the database:
...
| `vectors` | Embedding vectors for hybrid query reranking |

The database is created lazily on the first write. A corrupt file is deleted and recreated on open rather than failing the run, because every entry holds only derived data.

On first open after an upgrade from an older Fuse version, legacy directories `.fuse/cache` and `.fuse/index` are removed if present. They are not migrated entry by entry; the new database repopulates from scratch on the next run.
...
The vectors store keys embedding vectors by a caller-defined string (typically content hash plus model identity) when hybrid reranking and the persistent index are both enabled.
...
The `--index` flag enables the persistent analysis, postings, and vector stores. It is off by default for one-shot CLI runs and enabled automatically in watch mode and the MCP server, where a session makes several calls and the analysis cost is paid once.
