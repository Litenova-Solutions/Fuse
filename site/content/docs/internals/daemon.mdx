...
Without a resident workspace (the store-backed default), an oracle `fuse_check` answers from a captured compiler log by spawning a worker that rehydrates the compilation from the log, checks one file, and exits. In an agent loop that runs a check after every edit, that rehydrate-and-exit is repeated per check, several seconds each. R48 pools the worker: one long-lived worker per captured compiler log, kept alive across checks. It rehydrates the log once (reported by a ready handshake), then answers many check requests over its stdio - each speculative edit forks the in-memory document against the held compilation - so the second and later checks in a session skip the rehydrate. Measured on a captured log: spawn-per-call is about 1.7s per check (process launch plus rehydrate plus check), while a pooled session rehydrates once and answers subsequent checks near-instantly (three pooled checks totaled 1.3s, versus about 5s spawn-per-call); the saving grows with the log size.
