...
This page covers reducers for non-language formats. A reducer for a programming language belongs with its language plugin and may come with further capabilities such as skeleton extraction and dependency analysis; see [Adding a Language Plugin](/docs/internals/extending/language-plugin). Which files reach a reducer in the first place is governed by templates, covered in [Adding a Template](/docs/internals/extending/template).

## Next

The [Reducers reference](/docs/reference/reducers) documents each built-in reducer and the transforms it applies. The [contributing guide](/docs/project/contributing) covers coding standards and the pull request checklist.
