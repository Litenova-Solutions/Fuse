...
After registration the template is part of the internal collection preset set keyed by its enumeration member. Fuse applies presets internally by workspace type rather than through a command flag, so a registered template extends which file types the collection pipeline recognizes.
