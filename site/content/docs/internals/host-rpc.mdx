...
|---------|---------|---------|
| `fuse/handshake` | `protocolVersion` does not match `FuseHostService.ProtocolVersion` | Hook and daemon clients treat the host as absent (`IsServingAsync` is false; `TryCheckDeltaAsync` and `TryCheckOverlayAsync` return null). With `FUSE_HOOK_VERBOSE=1`, stderr logs `protocol_mismatch` on `fuse/handshake`. |
| Root-bound RPC (`fuse/index`, `fuse/graph`, `fuse/scope`, `fuse/explain`, `fuse/diagnostics`, `fuse/check`, `fuse/checkOverlay`) | `root` does not match the daemon's served root | JSON-RPC `InvalidParams`; message contains `served root`. |
...
| Store-backed MCP read after a storm reconcile (for example `fuse_impact`) | More than 300 known files are dirty at open | The leading availability header reads `index_state: stale_as_of`, names `files_indexed`, and reports `N known file(s) changed since index, results may lag the working tree`. |
