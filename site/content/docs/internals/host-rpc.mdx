...
The host is the local-IPC twin of `fuse mcp serve`. The MCP server exposes the warm engine to an agent over stdio; the host exposes it to a local client over an IPC transport, sharing the same semantic index, SQLite store, and engine path, so a read the host answers is the same read an agent would get for the same options. The ambient-verification hooks use it for the `fuse/check` delta; the remaining read methods are the general host surface a future editor client or the G5 daemon can consume.
...
| `fuse/check` | Returns the diagnostics a session's edits introduced or resolved since its baseline (the hook path, S3) |
...
| A local client (the ambient-verification hooks today) is the intended caller | No caller identity is checked today |
...
There is **no authentication** on the wire today beyond a per-session token (see below). The first RPC method a client calls is `fuse/handshake`, which returns version metadata and the session token. Every other method requires that token but is otherwise served to any connected peer with equal privilege: index, scope, diagnostics, check, and shutdown are all callable by a peer that has the token.
...
| Call `fuse/index` or `fuse/graph` | CPU and disk load; reads the working tree | Session token |
...
|---------|-----------|-----------------|
| `fuse host` | Named pipe or Unix socket | Session token issued at handshake |
