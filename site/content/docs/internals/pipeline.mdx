...
Several stages need the same file content. Filtering reads files to build the dependency graph and to index them for query ranking; reduction reads them to transform them. A single shared content provider reads each file once per run and serves that content to every consumer, so a file is not read from disk three times. Each run constructs its own content provider (and its own BM25 relevance index), so this state is run-local rather than shared between runs. Because no per-run state is shared, the orchestrator holds no process-wide gate: independent fusion runs execute concurrently and scale toward the core count.
...
Read the [Capability and Plugin Model](/docs/internals/capability-model) to see how reduction and scoping resolve language behavior, the [Options Model](/docs/internals/options-model) for how a request is described, or [Scoping Internals](/docs/internals/scoping-internals) for the filtering algorithms. For the conceptual overview, see [Core Concepts](/docs/concepts/how-fuse-works).
