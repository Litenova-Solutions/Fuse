...
Because a field-weight inversion is invisible without a metric, ranking is guarded by a recorded regression suite, run with `fuse eval ranking`. It scores the ranking with MRR, recall@k, and nDCG@k against changed-file ground truth in three configurations over the same index: the base channels in isolation (no embedder, both structural priors off), the shipping default (embedder if present, both priors on), and the shipping default with the git co-change prior off, which isolates that prior's effect on ranking. The suite is the required gate on any change to field weights, tokenization, query expansion, or priors, and it writes `results/ranking.json`.
