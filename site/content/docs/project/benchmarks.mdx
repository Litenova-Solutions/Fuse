...
To index the corpus semantically, add `--restore`, which runs `dotnet restore` on each checkout or review worktree before indexing:
...
A repository that cannot restore on the installed SDK stays in syntax mode and is recorded as such. Add `--require-semantic` to skip any checkout that does not reach semantic mode rather than silently scoring at the syntax fallback. Each suite writes its scorecard to `tests/benchmarks/results/<suite>.json` with per-task hits, misses, and the index-mode distribution. Model-driven suites are not byte-reproducible.
