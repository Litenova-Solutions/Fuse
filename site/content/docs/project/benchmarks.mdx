...
- Roslyn precision tier (`--semantic`). This is the accuracy result of the whole effort. Running `--level skeleton --semantic` over the same C#-only mirrors the harness uses, with the same independent Roslyn fidelity oracle, the precision tier keeps the signatures the regex skeleton drops:
