...
- Round-trip model (layer 3, illustrative). A simulated stateless agent records round-trips (K) and cumulative prefill tokens for three strategies: naive file-by-file exploration (which re-sends its whole context each turn, so cost grows with the square of the turns), a `fuse_toc` survey followed by one budgeted `fuse_search` (K=2), and a single budgeted `fuse_ask`-style retrieval (K=1). The token counts come from real Fuse runs, but the agent is a model, not a measured agent, so K and cumulative prefill are labeled illustrative of the round-trip structure rather than presented as benchmarks. A downstream task-success layer (feeding fused context to a fixed model and scoring answers) is gated behind an API key and is out of scope for the keyless harness run.
