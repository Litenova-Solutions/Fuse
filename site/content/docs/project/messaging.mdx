...
- Change-impact context (`review.json`, A3 corpus, 53 PRs): a PR's scoped context in a median 958 returned tokens at 100 percent changed-file recall and 79.8 percent precision.
...
- Token reduction (`reduction.json`): the Roslyn skeleton keeps essentially all public API while removing roughly 37 to 55 percent of tokens at skeleton level (a support number, not the headline).
