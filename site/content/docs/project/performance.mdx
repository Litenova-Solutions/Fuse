...
Tier-1 build capture (`dotnet build` then rehydrate from the log) is the higher-grade cold path, not the faster one. An A/B on eShopOnWeb (10 projects, environment-dependent, recorded in the R49 progress log) cold-indexed capture-on at ~16.4s versus the tier-2 MSBuildWorkspace path at ~13.6-15.0s: the build plus rehydrate costs a few seconds more than the design-time workspace load at this scale, for identical output. Build capture stays on by default because it produces the build-exact oracle graph (the substrate for `fuse_check`, `fuse_impact`, and covering-test selection), which is its justification - not a faster cold index.
...
For the store-backed default (no resident workspace), `fuse_check`'s oracle path keeps a pooled build-capture worker per captured compiler log (R48): it rehydrates the log once, then answers many checks against the held compilation, so the second and later checks in a session skip the rehydrate. Measured on a captured log: spawn-per-call is ~1.7s per check, while a pooled session rehydrates once and answers subsequent checks near-instantly (three pooled checks totaled ~1.3s versus ~5s spawn-per-call). Verdicts are identical to the spawn-per-call path (check honesty preserved).
...
The self-contained runtime binaries are published ReadyToRun (R50), so a one-shot CLI invocation (`fuse find`, `fuse context`, or the ambient-verification hooks that shell out per event) pays less JIT warmup on cold start. Measured on win-x64 (`fuse --version`, environment-dependent): ReadyToRun ~150 ms versus ~188 ms without it (best 146 vs 181 ms). Full AOT is not used (its native link is unavailable in the build environment); the long-lived daemon pays JIT once per session and is unaffected either way.
