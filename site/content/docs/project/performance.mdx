...
Reduction results are cached per file under `.fuse/cache`, so an unchanged file is not reduced again on a later run. The cache is on by default; disable it with `--no-cache` and clear it with `--clear-cache`. The cache key and eviction behavior are documented in [Caching Internals](/docs/internals/caching-internals).
...
Read [Benchmarks](/docs/project/benchmarks) for the full results, [Caching Internals](/docs/internals/caching-internals) for how the reduction cache decides what to reuse, or [Pipeline](/docs/internals/pipeline) for the stages that the parallelism and content-provider features apply to.
