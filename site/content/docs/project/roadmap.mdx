...
> Direction update. The forward program is now the resident verified-edit runtime (the compiler oracle), tracked item by item in `roadmap/v4.1-plan.md`. Two retrieval bets on this page are closed rather than planned: the dense embedding channel was retired (its recorded lift did not pay for a model dependency), and the learned-ranking and graph-verbalization bets are evidence-killed, since the tier-1 build-capture re-run showed no open-ended-recall lift. A richer build-exact graph is a verification asset (speculative check, blast radius, covering tests), not a retrieval lever. The embedding and learned-rerank rows below are retained as historical direction, not active plans.
