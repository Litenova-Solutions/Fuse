...
| Channel | How | Overhead | When |
| --- | --- | --- | --- |
| CI Action (recommended) | one workflow step runs `fuse capture` on `main` | one capture build per run | most repositories; the primary channel |
| Direct | run `fuse capture --out <bundle>` locally or in any pipeline | one capture build | ad hoc, or a nonstandard pipeline |
