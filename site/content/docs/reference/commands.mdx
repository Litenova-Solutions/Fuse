...
The .NET-specific flags and the three mutually exclusive scoping modes are documented in the [Options reference](/docs/reference/options). C# structural reduction is set with a single `--level` flag (`none`, `standard`, `aggressive`, `skeleton`, or `publicApi`). The same command also exposes the table-of-contents survey (`--toc`), generated-code collapse (`--collapse-generated`), the on-disk analysis store (`--index`), and the query reranker (`--rerank`). For task-oriented usage, see the [Scoping to What Matters](/docs/concepts/scoping) and [Reducing Tokens](/docs/concepts/reduction-levels) guides.
