...
The index lives in `.fuse/fuse.db` inside a repository, or under `FUSE_USER_DATA` outside one. Run `fuse index [path]` before one-shot CLI read commands. MCP read tools build the index on first use, except `fuse_workspace action=status` and the `fuse diagnostics` CLI command, which read `index_meta` only and return `not_indexed` without creating a database on a cold workspace. Running `fuse` with no command prints connection and workflow guidance.
...
Report the index state: schema version, mode, status, record counts, `index_state`, and the store location. Uses the same fast read-only path as `fuse_workspace action=status`: when `.fuse/fuse.db` is absent it reports `not_indexed` without creating the database.
