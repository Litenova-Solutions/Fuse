...
The index lives in `.fuse/fuse.db` inside a repository, or under `FUSE_USER_DATA` outside one. Run `fuse index [path]` before one-shot CLI read commands. MCP read tools build the index on first use. Running `fuse` with no command prints connection and workflow guidance.
