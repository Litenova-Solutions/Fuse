...
| `fuse localize [path]` | Rank candidate files and symbols for a task (no bodies) | yes | no |
| `fuse resolve [path]` | Resolve a service, request, route, config, or symbol | yes | no |
...
| `fuse find <query>` | Find an exact symbol, path, or text match | yes | no |
...
| `fuse mcp install` | Write MCP client registration for launching `fuse mcp serve`; optional `--rules` and `--with-hooks` write separate artifacts | no | no |
...
Build or refresh the persistent semantic index for a workspace. A path inside Git resolves to the repository root. The command loads the workspace through MSBuild/Roslyn when possible and falls back to syntax-level indexing otherwise. A normal refresh reconciles the complete inventory and prunes stale files; `--force` first discards all derived tables.
...
Accepts `--task`, `--route`, `--symbol`, `--service`, `--request`, `--config`, `--changed-since`, `--max-candidates`, `--strict`, and `--expand`. Best-effort localization is the default; `--strict` refuses an insufficient query and returns only its navigation map.
...
Run the covering tests for a symbol (the tests that reach it via persisted `tests` edges) at build grade, scoped by filter. Run `fuse index` first.
...
Report the index state: schema version, mode, status, record counts, `index_state`, and the store location. This command is deliberately read-only: when `.fuse/fuse.db` is absent it reports `not_indexed` without creating the database. In contrast, MCP `fuse_workspace action=status` validates or warms the repository index before reporting status.
...
`fuse init` writes a configuration file. `fuse mcp install` writes the selected client's MCP registration only. `--rules` also writes `AGENTS.md`, `CLAUDE.md`, or the client's documented instruction file; it does not install a skill. `--with-hooks` writes project-scoped Claude Code hooks and is refused with user scope. `fuse mcp serve` runs the stdio server (see [Connect your coding agent](/docs/start/connect-your-ai)). `fuse host` runs the long-lived repository host and refuses a path with no Git identity.
