...
| `FUSE_BG_UPGRADE` | `0` (off) | Opt-in syntax-first serving for `mcp serve`. Set to `1` (also `on` or `true`) to serve the syntax tier after the first pass and upgrade the semantic graph in the background. The explicit CLI `fuse index` is always a synchronous full pass. |
