...
Indexing scope: Fuse indexes the repository's own tracked and untracked-not-ignored source. In a git work tree it uses `git ls-files --exclude-standard`, so `.gitignore` is honored by construction; outside git it walks the tree. Either way it prunes the default excluded directory names, any directory that is a nested version-control root (a vendored checkout, worktree, or submodule with its own `.git`), and the `ignore` additions above. There is no silent file cap: the indexed file and symbol counts reported by `fuse_workspace action=status` reflect the real source set, rather than a truncated subset.
...
| `FUSE_COCHANGE` | `0` (off) | Git co-change collection at index time (R41). The git co-change prior is off in the shipping ranking (Decision D6, net-negative on the corpus), so mining and storing co-change on every index is wasted work - the `git log` walk was a large share of the index hot path. Off by default, so the default index path no longer runs the collector. Set to `1` (also `true`, `yes`, `on`) to collect (the ranking diagnostic that enables the prior sets this to gather the data it measures). |
...
| `FUSE_EAGER_INDEX` | `1` (on) | Eager warm-on-start indexing (R38): when a repo is served (`fuse mcp serve` in-process, or `fuse host`), the daemon kicks off a background syntax-first index the moment it starts, before any tool call, so the first read hits a warm or bounded-building index instead of paying the full cold cost. Set to `0` (also `false`, `no`, `off`) to opt out; the first read then builds on demand (still bounded, R27). `fuse warm <path>` warms explicitly regardless of this flag. |
...
| `FUSE_MAX_FILE_BYTES` | `5242880` (5 MB) | The per-file size cap for indexing (R35): a file larger than this is skipped with a recorded reason (a generated giant or a mislabeled binary costs memory and time for no signal) rather than read into the index. Skipped files are recorded under `skipped_files` in `index_meta` and shown by `fuse_workspace action=doctor`. A non-positive value disables the cap. |
