...
| `FUSE_BG_UPGRADE` | `0` (off) | In the `mcp serve` host, a cold first read serves the syntax tier in a few-to-twenty seconds and upgrades to the full semantic graph in the background, so the first answer does not block on the MSBuild load. Off by default (the first read indexes synchronously); set to `1` (also `on` or `true`) to enable the syntax-first cold start. Only affects the long-running server; the CLI `fuse index` is always a synchronous full pass. |
...
| `FUSE_DOWNGRADE_DROP` | `1` (on) | Downgrade before dropping when packing under a token budget: the lower-relevance tail that would be cut is replaced with a compact structural sketch instead, so a would-be-dropped file stays present. Applies to query and focus, not change mode. It only adds sketched files and never displaces a fuller one, so it cannot lower recall; it keeps a compact structural sketch of a would-be-dropped file, raising recall at tight budgets. Set to `0` (also `off` or `false`) to reproduce the drop-only packer. |
