...
| `FUSE_EMBEDDINGS` | unset | Embedding backend for `--rerank` when no explicit `--embeddings` flag is given: `1`/`true` selects the semantic ONNX model, `0`/`false` forces the hashing embedding. Unset uses the build default. The explicit flag wins over this variable. Falls back to hashing when the model is unavailable. |
