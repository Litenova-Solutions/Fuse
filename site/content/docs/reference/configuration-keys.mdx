...
| `FUSE_BG_UPGRADE` | `1` (on) | In the `mcp serve` host, a cold first read serves the syntax tier in a few-to-twenty seconds and upgrades to the full semantic graph (and tier-1) in the background, so the first answer does not block on the MSBuild load or the build. On by default; the availability header names when a build is running for tier-1. Set to `0` (also `off`, `no`, or `false`) to index synchronously on the first read. Only affects the long-running server; the CLI `fuse index` is always a synchronous full pass. |
| `FUSE_BUILD_CAPTURE` | `1` (on) | Tier-1 build capture: build the workspace once (through the bundled out-of-process worker) and rehydrate the exact Roslyn compilation, so `fuse_check` answers oracle-grade and the extracted graph is build-exact. On by default; it degrades cleanly to the MSBuild and syntax tiers when no build target exists or a build fails. Set to `0` (also `off`, `no`, or `false`) to skip build capture entirely. |
