...
|--------------|---------|---------|
| `fuse://map/{path}` | A workspace map: indexed symbols, routes, and counts | `fuse_map` |
| `fuse://localize/{path}/{query}` | Ranked candidate files and symbols for the task, no source bodies | `fuse_localize` |
...
The `{seed}` segment of the context resource is treated as a symbol seed; for service, request, route, or config seeds, or to pass several seeds, use the `fuse_context` tool. All four resources return content with the MIME type `text/plain`.
