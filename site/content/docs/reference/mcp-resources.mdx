...
This page documents the resource URIs and their default behavior. It does not cover the parameters that let you set token limits or reduction flags, which are available only on the tools. See [Tools Reference](/docs/reference/mcp-tools) for those.

## Next

Continue to [Tools Reference](/docs/reference/mcp-tools) when you need token limits, reduction flags, or exclusion control on a read.
