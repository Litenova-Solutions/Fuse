...
Read tools build the persistent index on first use, so an explicit `fuse_workspace action=index` call is optional. The exception is `fuse_workspace action=status` (and `fuse diagnostics` on the CLI): both read `index_meta` only, report `index_state`, and return `not_indexed` on a cold workspace without creating `.fuse/fuse.db`. The index lives in a single SQLite file at `.fuse/fuse.db` inside the workspace. Tools return their result in the tool response; the only writes are the index (`fuse_workspace action=index`) and the one explicit tree-write path (`fuse_workspace action=apply`, a dry run unless `write=true`). Token budgets are honored by the plan, and secret redaction runs before any source reaches the payload.
...
Each MCP tool belongs to one of three tiers. The tier names what must be available before the tool can answer,
not how fast it is.

| Tier | Tools | Needs `.fuse/fuse.db` |
|------|-------|------------------------|
| **offline** | `fuse_reduce` | No |
| **compiler** | `fuse_check`, `fuse_refactor`, `fuse_test` (build-grade verify, candidate racing, and `dotnet test` execution) | No for verification or test execution; the index is optional enrichment only |
| **indexed** | `fuse_workspace` (`index`, `map`, `doctor`, `apply`), `fuse_find`, `fuse_context`, `fuse_impact`, `fuse_review`, `fuse_test` (covering-test selection) | Yes (built on first use for read tools, or warm read when already present) |

`fuse_workspace action=status` and `fuse diagnostics` read `index_meta` only and never start a build.
...
tests from persisted `tests` edges (indexed-tier); when the store is contended it returns `index_busy:` rather than
blocking the subsequent `dotnet test` path on index build.
...
| `index_not_built:` | No index exists at `.fuse/fuse.db` (or it is empty). | Run `fuse_workspace action=index` (or `fuse index`) before indexed reads. |
...
| Value | Meaning |
|-------|---------|
| `not_indexed` | No `.fuse/fuse.db` or zero indexed files. |
...
| `stale_as_of` | A bulk change outran per-read reconcile; results may lag the working tree. |

`files_indexed` is omitted only when the store cannot be read (for example an exclusive SQLite lock). `fuse_workspace action=status` and `fuse diagnostics` report the same `index_state` values on the fast read-only path without starting a build on a cold workspace.
...
- **`status`** (default): the index mode, verification grade, freshness, and `index_state` (`not_indexed`, `building_syntax`, `upgrade_pending`, `ready`, `index_busy`, `stale_as_of`), plus `files_indexed` when known. This is a read-only fast path: it reads `index_meta` when `.fuse/fuse.db` exists and never starts a build on a cold workspace.
...
- **`doctor`**: the per-project semantic-load diagnosis (why a project loaded at the tier it did). The summary header uses the same fast `index_meta` read as `status`; the MSBuild load diagnosis still runs below it.
