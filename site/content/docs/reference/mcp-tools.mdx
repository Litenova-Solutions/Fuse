...
- **`status`** (default): validates or warms the repository index, then reports the index mode, verification grade, freshness, and `index_state` (`building_syntax`, `upgrade_pending`, `ready`, `index_busy`, `stale_as_of`), plus `files_indexed` when known.
