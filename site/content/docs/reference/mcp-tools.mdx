...
ranked files and their neighborhood, replacing a grep-and-open loop. Set `rerank` to true
to reorder the BM25 candidates by embedding-vector similarity (hybrid retrieval), an
opt-in tier; the default ranking is pure BM25F and uses no model.
