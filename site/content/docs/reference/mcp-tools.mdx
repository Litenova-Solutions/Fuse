...
diff, it has by far the highest recall of the files a task touches (87 percent on the benchmark
corpus, against 55 percent for query at the same budget), so routing PR-shaped work here rather
