...
Every retired or folded name (`fuse_index`, `fuse_map`, `fuse_localize`, `fuse_resolve`, `fuse_neighbors`, `fuse_signatures`, `fuse_changeset`, and the older V2 names) stays registered as a deprecation shim that returns a message naming its replacement, so a client that cached an older tool list gets clear guidance instead of an opaque Unknown tool error. Reconnect the MCP server to refresh the tool list.
...
- **`apply`**: write a proposed single-file edit (`file` plus `content`) to the working tree. This is the server's one explicit tree-write path (Decision D2): it is a dry run that only reports the change unless `write=true`, and it refuses any `file` that resolves outside the workspace root.
...
(The former `fuse_index` and `fuse_map` tools folded into this tool; their old names remain as deprecation shims.)

## Task localization (fuse_find kind=task)

Localize a task to ranked candidate files and symbols with `fuse_find` and `kind=task` (formerly the standalone `fuse_localize` tool, now folded). Returns candidates with reasons and token costs, no source bodies. The cheap first step of an iterative workflow; follow with `fuse_context`. Pass the task as `query`.
...
Deterministically resolve .NET wiring with `fuse_find`: `kind=service` (to its implementation), `request` (to its handler), `route` (to its action), or `config` (to its options type). No source bodies. Pass the identifier as `query`. (Formerly the standalone `fuse_resolve` tool, now folded; the old name remains a deprecation shim.)
...
The review also carries a graded claims block (U2): the changed-file set (git-truth, so `verified`) and, when the surface moved, a `partially verified` claim that the change alters the public API. Each claim names its evidence. See [claim grades](/docs/concepts/claim-grades) for the grade table and the session ledger.
...
- **Wiring** (`kind=service|request|route|config`): resolve the query to its implementation, handler, action, or options type (folds the former `fuse_resolve`).
- **`kind=signatures`**: the query symbol's exact signature (folds `fuse_signatures`). When a resident workspace serves the root (`FUSE_RESIDENT`), a qualified name (a namespace-qualified type, or `Type.Member`) resolves from the compiler's real metadata, so a referenced package's API answers from the compiler rather than the store - the hallucinated-package-API check. Without a resident workspace, or for a name it does not resolve, the store-backed signature index answers.
- **`kind=neighbors`**: the query symbol's callers and implementers, ranked with provenance (folds `fuse_neighbors`).
- **`kind=task`**: rank candidate files and symbols for the query, with the graded refuse-and-route contract (folds `fuse_localize`; see [The graded response](#the-graded-response)).
...
(The former `fuse_localize`, `fuse_resolve`, `fuse_neighbors`, and `fuse_signatures` tools folded into this tool; their old names remain as deprecation shims.)
...
Look up a symbol's exact signature (declared signature, kind, accessibility, containing type, and location) with `fuse_find` and `kind=signatures`, the compiler-shaped answer to "what is the exact shape of this member" instead of grep-and-read. A signature is recorded at the semantic tier; in syntax mode a symbol may have no recorded signature, and the tool says so rather than inventing one. Pass the symbol name as `query`. (Formerly the standalone `fuse_signatures` tool, now folded; the old name remains a deprecation shim. Answering over referenced-assembly metadata when a resident workspace serves the root is a tracked follow-up.)

## The propose-verify-apply flow (fuse_changeset dissolved)

The speculative staging workflow dissolved in the eight-tool reshape: instead of a stateful changeset session, the loop is direct. Diagnose a proposed single-file edit by passing its content to `fuse_check` (the speculative typecheck, oracle-grade when tier-1 is available). Make compiler-executed edits with `fuse_refactor` (each returns a verified staged diff or abstains). Select the covering tests for a changed symbol with `fuse_impact` (which lists them) or run them with `fuse_test`. When you choose to commit, write the result to the working tree with `fuse_workspace action=apply` (the one explicit write path, a dry run unless `write=true`). Nothing is written until that apply. (The old `fuse_changeset` name remains a deprecation shim naming this flow.)
