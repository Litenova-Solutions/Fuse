...
| `--embeddings` | bool | unset | Embedding backend for `--rerank`: on for the semantic ONNX model (downloaded once to `~/.fuse/models`), off for the dependency-free hashing embedding. Unset uses the build default. Ignored in the Native AOT build, which always uses hashing. |
