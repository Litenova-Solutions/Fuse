...
| Flag | Type | Default | Description |
|------|------|---------|-------------|
| `--max-tokens` | int | None | Hard cap on the total payload. For a scoped run (`--query` or `--focus`) the packer reserves room for the manifest, route and project maps, redaction report, pattern summary, and any preamble, so the complete output fits the budget rather than overrunning it by the size of that framing. The single most-relevant file is always emitted, even if it alone exceeds the cap. |
...
| `--dedup-bodies` | bool | `false` | Replace member bodies that are byte-identical across two or more files (after whitespace normalization) with a marker referencing the canonical copy, emitted once. The member signature is always preserved. |
...
The kinds of secret Fuse detects are listed in the [Secret Redaction Kinds reference](/docs/reference/secret-redaction-kinds). Redaction is best-effort pattern and entropy matching; it lowers accidental exposure but does not guarantee every secret is removed. Do not rely on it for compliance or secret scanning.
...
| `--clear-cache` | bool | `false` | Clear the `reduction` namespace in `.fuse/fuse.db` before running. |
| `--index` | bool | `false` | Cache per-file analysis, relevance tokens, and optional vectors on disk (`.fuse/fuse.db`) so repeated scoped runs reuse them. Enabled automatically in watch mode and the MCP server. |
...
| Flag | Type | Default | Description |
|------|------|---------|-------------|
| `--focus` | string | None | Type name, filename, or relative directory to scope the fusion around. A `Type.Member` seed scopes the seed file to that member (symbol-level scoping). |
| `--query` | string | None | Search query to scope fusion to the most relevant files. For C# the query ranks at member granularity and emits a thin host-type skeleton with the matched members in full and the rest collapsed to signatures (symbol-level retrieval). |
