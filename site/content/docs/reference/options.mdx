...
| `--embeddings` | bool | unset | Embedding backend for `--rerank`: on for the semantic ONNX model (downloaded once to `~/.fuse/models`), off for the dependency-free hashing embedding. Unset uses the build default. Falls back to hashing when the model is unavailable. |
