...
The format reducers handle web and configuration file types. Each strips comments and collapses whitespace in a way appropriate to its syntax. They are controlled by two options: `--minify-xml-files` governs the XML-family reducers and `--minify-html-and-razor` governs the HTML and Razor reducers; both default to true. See the [Options reference](/docs/reference/options).
...
The C# reducer handles `.cs` files. Unlike the format reducers, its transforms are driven by a single reduction level (`--level`), so the same reducer produces output ranging from light cleanup to compressed source. Its steps fall into two tiers, each selected by a level.
...
Every string and character literal, including raw string literals (`"""..."""`, the four-quote form, and interpolated raw strings `$"""..."""`), is masked behind a placeholder before any removal or whitespace pass runs, then restored verbatim. Embedded JSON, SQL, and templates inside a literal survive reduction byte-identical at all levels, and a `//` or `/* */` sequence inside a raw string is never mistaken for a comment.

Aggressive mode maximizes token savings but produces output that is no longer guaranteed to compile. `--level aggressive` enables the full standard removal set together with the aggressive tier in one value. The deeper `--level skeleton` and `--level publicApi` drop method bodies entirely, emitting signatures only.
