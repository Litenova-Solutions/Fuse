...
This page does not give guidance on responding to a detected secret, rotating credentials, or treating redaction as a security boundary; the [Secret Redaction](/docs/scenarios/keep-secrets-out) guide covers operational use. It does not document the report format beyond stating that it counts matches per kind. For the flags named here, see [Options](/docs/reference/options).
