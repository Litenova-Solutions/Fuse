...
The extension list a template provides is a default. You can add, remove, or replace it per run with `--include-extensions`, `--exclude-extensions`, and `--only-extensions`, documented in the [Options reference](/docs/reference/options).
