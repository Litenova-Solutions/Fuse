...
In V3 these templates are internal collection presets: there is no command flag to select one. The indexing and collection pipeline applies the .NET defaults for a .NET workspace, and `fuse reduce` selects a reducer by extension. The tables below document the defaults each preset provides so you can see exactly which files are collected and which are skipped.
...
This page lists the defaults each template provides; it does not cover the order in which collection applies templates, `.gitignore` rules, and command-line overrides, which the [Core Concepts](/docs/concepts/how-fuse-works) page describes as the Collection stage. It does not document how to register a new template; the [Adding A Template](/docs/internals/extending/template) page covers that.

## Next

See the [Options reference](/docs/reference/options) to override a template's extensions for a single run, or the [Fusing .NET Code](/docs/scenarios/cut-tokens-dotnet) guide for the DotNet template in practice.
