...
These templates are internal collection presets: there is no command flag to select one. The indexing and collection pipeline applies the .NET defaults for a .NET workspace, and `fuse reduce` selects a reducer by extension. The tables below document the defaults each preset provides so you can see exactly which files are collected and which are skipped.
