...
drag on editing. Set `FUSE_HOOK_VERBOSE=1` when debugging hook wiring to log swallowed RPC
failures to stderr (method name and error code); leave it off in normal use. This is why
ambient verification needs a resident host: the delta is read from the live compilation
the host holds, with no rebuild.
