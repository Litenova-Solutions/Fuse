---
title: Why use Fuse
description: How Fuse adds compiler-backed edit checks, .NET wiring lookup, and focused branch context to an existing coding agent.
---

Before your coding agent changes `OrderService.cs`, have the compiler check the proposed
file:
...
Fuse adds local .NET checks and lookups to the editor and agent workflow you already use.
It does not replace file editing, exact text search, or `dotnet build`.
