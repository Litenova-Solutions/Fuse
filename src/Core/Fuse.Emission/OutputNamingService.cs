...
public sealed class OutputNamingService
{
    ...
    /// <param name="part">The part number.</param>
    /// <param name="tokenCount">The token count for this part.</param>
    /// <param name="overwrite">Whether to overwrite an existing file at the computed path.</param>
    /// <param name="isMultiPart">Whether this file is part of a multi-part set.</param>
    /// <returns>The full path to the finalized file.</returns>
    /// <remarks>
    ///     Performs disk side effects: when a file already exists at the target path it is deleted if
    ///     <paramref name="overwrite" /> is <c>true</c>, otherwise a timestamped fallback name is chosen to
    ///     avoid the collision. The temporary file is then moved into place via <see cref="File.Move(string, string)" />.
    /// </remarks>
