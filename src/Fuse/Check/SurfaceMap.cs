...
internal sealed record SurfaceEntry(string Signature, Reach Reach, string[] Names);
...
internal static class SurfaceMap
{
    public static Dictionary<string, SurfaceEntry> Compute(SyntaxNode root)
    {
        ...
        if (root is CompilationUnitSyntax unit)
        {
            foreach (var global in unit.Usings.Where(u => u.GlobalKeyword.IsKind(SyntaxKind.GlobalKeyword)))
                map["G:" + Flat(global)] = new SurfaceEntry(Flat(global), Reach.Broad, []);
            foreach (var list in unit.AttributeLists)
                map["A:" + Flat(list)] = new SurfaceEntry(Flat(list), Reach.Broad, []);
        }
            ...
            switch (child)
            {
                ...
                case DelegateDeclarationSyntax del:
                    map[$"T:{Qualify(container, del.Identifier.Text)}`{Arity(del.TypeParameterList)}"] =
                        new SurfaceEntry(Flat(del), Reach.Broad, [del.Identifier.Text]);
                    break;
    ...
    private static void AddType(BaseTypeDeclarationSyntax type, string container, Dictionary<string, SurfaceEntry> map)
    {
        ...
        map["T:" + qualified] = new SurfaceEntry(header.ToString(), Reach.ByName, [name]);

        if (type is EnumDeclarationSyntax enumDeclaration)
        {
            foreach (var member in enumDeclaration.Members)
                map[$"F:{qualified}.{member.Identifier.Text}"] = new SurfaceEntry(Flat(member), Reach.ByName, [member.Identifier.Text, name]);
            return;
        }
        ...
        switch (member)
        {
            ...
            case DelegateDeclarationSyntax del:
                map[$"T:{qualified}.{del.Identifier.Text}`{Arity(del.TypeParameterList)}"] =
                    new SurfaceEntry(Flat(del), Reach.Broad, [del.Identifier.Text, typeName]);
                break;
            ...
            case ConstructorDeclarationSyntax ctor:
                map[$"C:{qualified}({ParameterTypes(ctor.ParameterList)})"] =
                    new SurfaceEntry($"{prefix} {Flat(ctor.ParameterList)}", Reach.ByName, [typeName]);
                break;
            case PropertyDeclarationSyntax property:
                map[$"P:{qualified}.{property.ExplicitInterfaceSpecifier}{property.Identifier.Text}"] =
                    new SurfaceEntry($"{prefix} {Flat(property.Type)} {property.Identifier.Text} {Accessors(property.AccessorList, property.ExpressionBody)}", Reach.ByName, [property.Identifier.Text, typeName]);
                break;
            case IndexerDeclarationSyntax indexer:
                map[$"I:{qualified}[{ParameterTypes(indexer.ParameterList)}]"] =
                    new SurfaceEntry($"{prefix} {Flat(indexer.Type)} {Flat(indexer.ParameterList)} {Accessors(indexer.AccessorList, indexer.ExpressionBody)}", Reach.Broad, [typeName]);
                break;
            case EventDeclarationSyntax evt:
                map[$"E:{qualified}.{evt.Identifier.Text}"] = new SurfaceEntry($"{prefix} {Flat(evt.Type)}", Reach.ByName, [evt.Identifier.Text, typeName]);
                break;
            case EventFieldDeclarationSyntax eventField:
                foreach (var variable in eventField.Declaration.Variables)
                    map[$"E:{qualified}.{variable.Identifier.Text}"] = new SurfaceEntry($"{prefix} {Flat(eventField.Declaration.Type)}", Reach.ByName, [variable.Identifier.Text, typeName]);
                break;
            case FieldDeclarationSyntax field:
                ...
                foreach (var variable in field.Declaration.Variables)
                {
                    ...
                    map[$"F:{qualified}.{variable.Identifier.Text}"] = new SurfaceEntry($"{prefix} {Flat(field.Declaration.Type)} {value}", Reach.ByName, [variable.Identifier.Text, typeName]);
                }

                break;
            case OperatorDeclarationSyntax op:
                map[$"O:{qualified}.{op.OperatorToken.Text}({ParameterTypes(op.ParameterList)})"] =
                    new SurfaceEntry($"{prefix} {Flat(op.ReturnType)} {Flat(op.ParameterList)}", Reach.Broad, [typeName]);
                break;
            case ConversionOperatorDeclarationSyntax conversion:
                map[$"O:{qualified}.{conversion.ImplicitOrExplicitKeyword.Text}({Flat(conversion.Type)})"] =
                    new SurfaceEntry($"{prefix} {Flat(conversion.ParameterList)}", Reach.Broad, [typeName]);
                break;
        }
    }
