# Spike: compare query-mode recall with budget-aware expansion ON vs OFF (item 4), across all three
# budgets. Fast, throwaway A/B over the same PRs layer2a uses, query mode only. Budget-aware expansion is
# meant to be recall-neutral (a cost gate that mirrors the packer's score order, saving reduction work), so
# this checks that the default-on change does not regress recall at any budget or for any repo.
...
function Run-Mode($wt, $q, $budget, $budgetAware) {
    $outDir = Join-Path $ResultsDir ".spike/$([guid]::NewGuid().ToString('N'))"
    New-Item -ItemType Directory -Force -Path $outDir | Out-Null
    $env:FUSE_BUDGET_EXPANSION = $budgetAware
...
foreach ($g in $repoGroups) {
    ...
    foreach ($pr in $g.Group) {
        $wt = Join-Path $ResultsDir ".wt/spikeb_$($g.Name)_$($pr.pr)"
...
Remove-Item Env:\FUSE_BUDGET_EXPANSION -ErrorAction SilentlyContinue
