# Spike: compare query-mode recall with the git churn prior ON vs OFF (Q6) at the headline budget. The on arm
# sets FUSE_GIT_CHURN_WEIGHT=0.15 so a recently and frequently changed candidate is nudged up; off is the
# lexical floor. EXPECTED NEUTRAL, and that is the point: the corpus worktrees are historical PR-head
# checkouts, so churn-from-now is uniformly empty (the prior is a no-op) and a date-relative churn would leak
# (the changed files are the most recently changed by construction). This confirms the prior does not chase or
# leak the benchmark; it is a production-routing lever. Prints per-repo and overall deltas. Not committed.
...
foreach ($g in $repoGroups) {
    ...
    foreach ($pr in $g.Group) {
        $wt = Join-Path $ResultsDir ".wt/spikec_$($g.Name)_$($pr.pr)"
        ...
        foreach ($b in $Budgets) {
            ...
            $on  = Measure-Recall (Run-Mode $wt $q $b '0.15') $truth
...
Remove-Item Env:\FUSE_GIT_CHURN_WEIGHT -ErrorAction SilentlyContinue
