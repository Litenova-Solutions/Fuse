# Spike: compare query-mode recall with downgrade-before-drop ON vs OFF (P1), across all three budgets. The on
# arm sets FUSE_DOWNGRADE_DROP=1 so a file the budget would drop is kept as a compact sketch instead; off drops
# it. Recall counts file presence, so this should help most at the tight 10k/25k budgets where truncation
# bites. Reports per-budget per-repo and overall recall deltas; keep as a default only if it lifts recall with
# no per-repo regression at 50k. Not committed results.
...
function Run-Mode($wt, $q, $budget, $downgrade) {
    $outDir = Join-Path $ResultsDir ".spike/$([guid]::NewGuid().ToString('N'))"
    New-Item -ItemType Directory -Force -Path $outDir | Out-Null
    $env:FUSE_DOWNGRADE_DROP = $downgrade
...
foreach ($g in $repoGroups) {
    ...
    foreach ($pr in $g.Group) {
        $wt = Join-Path $ResultsDir ".wt/spiked_$($g.Name)_$($pr.pr)"
...
Remove-Item Env:\FUSE_DOWNGRADE_DROP -ErrorAction SilentlyContinue
