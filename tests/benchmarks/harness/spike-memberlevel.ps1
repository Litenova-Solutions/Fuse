# Spike: compare query-mode recall with member-level retrieval ON vs OFF (Q5), across all three budgets. The
# on arm sets FUSE_MEMBER_LEVEL=1 so each declared member is indexed and rolled up to a file score, surfacing a
# file whose match is concentrated in one member of a large file as an extra seed; off is the lexical floor.
# Keep as a default only if it lifts recall with no per-repo regression at the 50k headline. Prints per-budget
# per-repo and overall recall deltas. Not committed results.
...
function Run-Mode($wt, $q, $budget, $memberlevel) {
    $outDir = Join-Path $ResultsDir ".spike/$([guid]::NewGuid().ToString('N'))"
    New-Item -ItemType Directory -Force -Path $outDir | Out-Null
    $env:FUSE_MEMBER_LEVEL = $memberlevel
...
foreach ($g in $repoGroups) {
    ...
    foreach ($pr in $g.Group) {
        $wt = Join-Path $ResultsDir ".wt/spikem_$($g.Name)_$($pr.pr)"
...
Remove-Item Env:\FUSE_MEMBER_LEVEL -ErrorAction SilentlyContinue
