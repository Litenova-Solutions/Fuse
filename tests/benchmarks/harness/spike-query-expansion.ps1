# Spike: compare query-mode recall with pseudo-relevance feedback expansion ON vs OFF.
# Fast, throwaway A/B over the same PRs layer2a uses, query mode only, headline budget only.
# Not committed results; prints a per-repo and overall recall delta so a tuning change can be
# judged before paying for a full layer2a run.
...
foreach ($g in $repoGroups) {
    ...
    foreach ($pr in $g.Group) {
        $wt = Join-Path $ResultsDir ".wt/spike_$($g.Name)_$($pr.pr)"
        ...
        $off = Measure-Recall (Run-Mode $wt $q '0') $truth
        $on  = Measure-Recall (Run-Mode $wt $q '1') $truth
        $rows += [pscustomobject]@{ repo=$g.Name; pr=$pr.pr; off=$off; on=$on; delta=[math]::Round($on-$off,3) }
        Write-Host ("  {0} PR#{1,-5} off {2:P0}  on {3:P0}  d {4:+0%;-0%; 0%}" -f $g.Name, $pr.pr, $off, $on, ($on-$off))

        git -C $repoPath worktree remove --force $wt 2>$null
        Remove-Item -Recurse -Force $wt -ErrorAction SilentlyContinue
    }
}
Remove-Item Env:\FUSE_QUERY_EXPANSION -ErrorAction SilentlyContinue

Write-Host "`n=== Per-repo mean recall ==="
...
$aoff = ($rows | Measure-Object off -Average).Average
$aon  = ($rows | Measure-Object on  -Average).Average
