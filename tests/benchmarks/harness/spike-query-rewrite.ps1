# Spike (item 12): heuristic PascalCase query rewrite ON vs OFF, query mode, all budgets. Not committed.
# A/B over the same PRs layer2a uses, query mode only. The on arm sets FUSE_QUERY_REWRITE=1 so comments and
...
Remove-Item Env:FUSE_QUERY_REWRITE -ErrorAction SilentlyContinue
