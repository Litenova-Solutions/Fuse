# Spike: compare query-mode recall with dense rerank ON vs OFF (item 9), across all three budgets. Throwaway
# A/B over the same PRs layer2a uses, query mode only. The on arm sets FUSE_RERANK=1 so the ONNX MiniLM
# reranker reorders a widened BM25 candidate pool and selects the seeds; the off arm is the lexical floor.
# The decision bar (per the plan) is whether rerank clears about 60 percent on the hard repos (AutoMapper,
# NewtonsoftJson). Prints per-budget per-repo and overall recall deltas. Not committed results.
