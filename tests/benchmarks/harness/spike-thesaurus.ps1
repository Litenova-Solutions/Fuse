# Spike: compare query-mode recall with the distributional thesaurus ON vs OFF (Q4), across all three budgets.
# The on arm sets FUSE_THESAURUS=1 so a query identifier is expanded with its corpus PMI-associated identifiers
# before ranking; off is the lexical floor. Keep as a default only if it lifts recall with no per-repo
# regression at the 50k headline. Prints per-budget per-repo and overall recall deltas. Not committed results.
...
function Run-Mode($wt, $q, $budget, $thesaurus) {
    $outDir = Join-Path $ResultsDir ".spike/$([guid]::NewGuid().ToString('N'))"
    New-Item -ItemType Directory -Force -Path $outDir | Out-Null
    $env:FUSE_THESAURUS = $thesaurus
...
foreach ($g in $repoGroups) {
    ...
    foreach ($pr in $g.Group) {
        $wt = Join-Path $ResultsDir ".wt/spiket_$($g.Name)_$($pr.pr)"
...
Remove-Item Env:\FUSE_THESAURUS -ErrorAction SilentlyContinue
